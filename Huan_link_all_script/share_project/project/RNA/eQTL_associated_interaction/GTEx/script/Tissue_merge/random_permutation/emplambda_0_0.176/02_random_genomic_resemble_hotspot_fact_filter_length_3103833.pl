...
for (my $i=1;$i<10001;$i++){
    ...
    my $command1 = "bedtools shuffle -i $input_file2 -g $genome -excl $anno_dir/complement_final_0_0.176_sorted_merge.bed.gz -chrom | gzip >$out_file"; #即用$basic_simulate 除去hotspot extend 的部分 进行随机抽样
    # print "$command\n";
    system $command1;
    print "$i\n";
}
