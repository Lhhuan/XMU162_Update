...
# pdf("./figure/count/compare_emplambda0_active_mark_count.pdf",width=8.3, height=4)
# CombinePlots(plist,ncol=4,nrow=2)
# dev.off()

# plist = lapply(repressed_mark,large)

# pdf("./figure/count/compare_emplambda0_repressed_mark_count.pdf",width=4, height=2)
# CombinePlots(plist,ncol=2)
# dev.off()

plist = lapply(all_histone_marker,large)

pdf("./figure/fraction/compare_emplambda0_all_histone_marker_fraction.pdf",width=8.3, height=4)
CombinePlots(plist,ncol=4,nrow=2)
dev.off()
...
pdf("./figure/fraction/compare_emplambda0_all_histone_marker_tfbs_fraction.pdf",width=8.6, height=6)
CombinePlots(plist,ncol=4,nrow=3)
dev.off()

plist = lapply(other_marker,large)
pdf("./figure/fraction/compare_emplambda0_other_tfbs_fraction.pdf",width=6, height=2)
...
pdf("./figure/fraction/compare_emplambda0_repressed_mark_histone_marker_fraction.pdf",width=6.5, height=2)
CombinePlots(plist,ncol=3)
dev.off()
...
write.table(final_result,"./figure/fraction/compare_emplambda0_all_histone_marker_tfbs_fraction_fold_change.txt",row.names = F, col.names = T,quote =F,sep="\t")
