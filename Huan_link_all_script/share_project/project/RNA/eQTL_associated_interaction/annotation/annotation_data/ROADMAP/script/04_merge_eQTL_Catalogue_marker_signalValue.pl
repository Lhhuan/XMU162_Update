...
foreach my $marker(@markers){
    my $fo1 = "${output_dir}/${marker}_signalValue.bed.gz";
    # open my $O1, '>', $fo1 or die "$0 : failed to open output file '$fo1' : $!\n";
    open my $O1, "| gzip >$fo1" or die $!;
    my $fo2 = "${output_dir}/${marker}_signalValue_sorted.bed.gz";  


    my $f1 = "all_eQTL_Catalogue_sample_id_unique.txt";
    ...
    while(<$I1>)
    {
        ...
        if(-e $f2){
            ...
            while(<$I2>)
            {
                ...
                my $end = $f[2];
                my $signalValue =$f[6];
                print $O1 "$chr\t$start\t$end\t$signalValue\n";
            }
        }
    }
    close($f1);
