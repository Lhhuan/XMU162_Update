...
#--------------------------------
    ...
    close($O2);
    close($O3);
    my $fo4 = "${output_dir}/merge_pos_info_narrow_peak_sort.bed.gz";
    my $fo5 = "${output_dir}/merge_pos_info_narrow_peak_sort_union.bed.gz";
    ...
    system "bedtools merge -i $fo4 |gzip > $fo5";
    print "$dir\n";
}
