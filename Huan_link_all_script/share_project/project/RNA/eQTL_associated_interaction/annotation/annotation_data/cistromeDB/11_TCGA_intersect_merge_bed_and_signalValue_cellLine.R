...
library(tibble)
library(R.utils)
setwd("/share/Projects/huanhuan/project/RNA/eQTL_associated_interaction/annotation/annotation_data/cistromeDB/cancer_cell/cell_type_level/")

ca <- read.table("../082_final_ca_tf_TCGA_sample.txt",header = T,sep = "\t") %>% as.data.frame()
histone <- read.table("../092_final_histone_TCGA_mark.txt",header = T,sep = "\t") %>% as.data.frame()
all_marker <- bind_rows(ca,histone)
all_marker <-all_marker[,c("TCGA","factor","cell_line_refine","cell_line")]%>%unique()

GRCh38_chrom_size <- "/share/Projects/huanhuan/ref_data/gencode/GRCh38_chrom.sizes"
chr_set <-paste0("chr",c(1:22))

for (i in 1:nrow(all_marker)){
    TCGA <-all_marker[i,"TCGA"]
    marker <- all_marker[i,"factor"]
    cell_line <- all_marker[i,"cell_line_refine"]
    path <-paste0("/share/Projects/huanhuan/project/RNA/eQTL_associated_interaction/annotation/annotation_data/cistromeDB/cancer_cell/cell_type_level/",TCGA,"/",cell_line)
    setwd(path)
