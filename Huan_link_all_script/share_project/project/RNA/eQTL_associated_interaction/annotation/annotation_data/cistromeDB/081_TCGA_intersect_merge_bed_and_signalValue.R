...
ctf <- read.table("ca_tf_TCGA_sample.txt",header = T,sep = "\t") %>% as.data.frame()
ctf <- unique(ctf[,c("marker","TCGA")])
ctf$marker_path <- paste0("./",ctf$marker,"/",ctf$TCGA)

markers = ctf$marker_path
