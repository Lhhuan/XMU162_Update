...
sub printCMD {
	...
	print STDERR "\t\t-gid (Use gene_id instead of transcript_id to identify the transcripts from GTF files)\n";
	print STDERR "\t\t-tid (Use transcript_id instead of gene_id to identify the transcripts from GTF files, default)\n";
	print STDERR "\t\t-ensemblRepeats <GFF3 file> (gff3 annotations from ensembl usually have repeat definitions too)\n";
...
for (my $i=0;$i<@ARGV;$i++) {
	if ($ARGV[$i] eq '-name') {
		...
		$org = $ARGV[++$i];
	} elsif ($ARGV[$i] eq '-ensemblRepeats') {
		$ensemblRepeatGFF3 = $ARGV[++$i];
...
if ($repeatsFile ne '' || $ensemblRepeatGFF3 ne '') {
	...
	if ($repeatsFile ne '') {
		...
		while (<IN>) {
			...
			my $str = "$name\t$line[1]\t$line[2]\t$line[3]\t$line[4]\t$family\t0\n";
			print OUT $str;
			if (!exists($type{$family})) {
				my @a = ();
				$type{$family} = \@a;
			}
			push(@{$type{$family}}, $str);
		}
		close IN;
		close OUT;
	} elsif ($ensemblRepeatGFF3 ne '') {
		open OUT, ">>$genomeDir/$genome.ann";
		open IN, $ensemblRepeatGFF3;
		while (<IN>) {
			chomp;
			s/\r//g;
			my @line = split /\t/;
			next if (@line < 9);
			next unless ($line[2] eq 'repeat_region');
			my $r = '';
			if ($line[8] =~ /description=([^;]+)/) {
				$r = $1;
			}
			my $name = $r;
			$r =~ s/_(.+)$//;
			...
			my $str = "$name|$r\t$line[0]\t$line[3]\t$line[4]\t$dir\t$r\t0\n";
      		print OUT $str;
			push(@{$type{$r}}, $str);
		}
		close IN;
	...
	foreach(keys %type) {
		my $r = $_;
		open OUT, ">$genomeDir/annotations/repeats/$r.ann.txt";
		foreach(@{$type{$r}}) {
			print OUT $_;
		}
		close OUT;
