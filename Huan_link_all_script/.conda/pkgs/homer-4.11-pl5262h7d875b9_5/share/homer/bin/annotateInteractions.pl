#!/usr/bin/env perl
use warnings;
use lib '/opt/conda/conda-bld/homer_1625584979754/_h_env_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placeho/share/homer/.//bin';
my $homeDir = '/opt/conda/conda-bld/homer_1625584979754/_h_env_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placehold_placeho/share/homer/./';
...
sub printCMD {
	...
	print STDERR "\t\t-ctcf <CTCF peakfile> <CTCF motif> (calculated directionality stats)\n";
...
for (my $i=3;$i<@ARGV;$i++) {
	if ($ARGV[$i] eq '-res') {
		...
		$poutFlag=1;
	} elsif ($ARGV[$i] eq '-ctcf') {
		$ctcfPeakFile = $ARGV[++$i];
		$ctcfMotifFile = $ARGV[++$i];
...
if ($ctcfPeakFile ne '' && $ctcfMotifFile ne '') {
	analyzeMotifDirection($interactions, $ctcfPeakFile, $ctcfMotifFile);
}
...
sub printInteractions {
	...
	foreach(@names) {
		...
		my $ucsc = getUCSClocation($interactions->{$n});
		print OUT "\t$ucsc";
		print OUT "\n";
	}
	close OUT;
	...
	foreach(@names) {
		...
		if ($int->{'c1'} eq $int->{'c2'}) {
			...
			if ($int->{'s1'} <= $int->{'s2'}) {
				...
				print OUT "$int->{'c1'}\t$start\t$end2\t$intname\t$score\t$int->{'d1'}"
						. "\t$start\t$start\t0";
				if ($int->{'e1'} < $int->{'s2'}) {
					my $size1 = $end-$start;
					my $size2 = $end2-$start2;
					my $start1 = 0;
					my $start2 = $start2-$start;
				...
				print OUT "$int->{'c1'}\t$start2\t$end\t$intname\t$score\t$int->{'d2'}"
						. "\t$start2\t$start2\t0";
				if ($int->{'e2'} < $int->{'s1'}) {
					my $size1 = $end2-$start2;
					my $size2 = $end-$start;
					my $start1 = 0;
					my $start2 = $start-$start2;
					print OUT "\t2\t$size1,$size2\t$start1,$start2\n";
				} else {
					my $size1 = $end-$start2;
					print OUT "\t1\t$size1,\t0\n";
			...
			my $size1 = $end-$start;
			$intName = $n . '->' . "$int->{'c2'}:$start2" . "-" . $end2;
			print OUT "$int->{'c1'}\t$start\t$end\t$intname\t$score\t$int->{'d1'}"
					. "\t$start\t$start\t0\t1\t$size1\t0\n";

			$size1 = $end2-$start2;
			$intName = $n . '->' . "$int->{'c1'}:$start" . "-" . $end;
			print OUT "$int->{'c2'}\t$start2\t$end2\t$intname\t$score\t$int->{'d2'}"
					. "\t$start2\t$start2\t0\t1\t$size1\t0\n";
...
sub analyzeMotifDirection {
	my ($ints, $ctcfPeakFile, $ctcfMotifFile) = @_;

	unless (-e $ctcfMotifFile) {
		print STDERR "!! Error - could not find motif file $ctcfMotifFile\n";
		return;
	}
	if (-e $ctcfPeakFile) {
		print STDERR "\tUsing peak file $ctcfPeakFile for motif direction detection at $centeringPeakSize resolution\n";
		`annotatePeaks.pl "$ctcfPeakFile" $genome -center "$ctcfMotifFile" -size $centeringPeakSize $mscoreFlag > "$tmpFile3"`;
		#print STDERR "`annotatePeaks.pl $ctcfPeakFile $genome -center $ctcfMotifFile -size $centeringPeakSize $mscoreFlag > $tmpFile3`;\n";
	} else {
		print STDERR "\tCould not find peak file for motif direction - using interaction endpoints at $res resolution\n";
		`annotatePeaks.pl "$intPeakFile" $genome -center "$ctcfMotifFile" -size $res $mscoreFlag > "$tmpFile3"`;
	}
	...
	while (<IN>) {
		...
		s/\r//g;
		my @line = split /\t/;
		$direction{$line[0]} = $line[4];
	}
	close IN;

	`mergePeaks -d $halfRes -cobound 1 "$leftPeakFile" "$tmpFile3" -prefix "$tmpFile4" 2> /dev/null`;
	`mergePeaks -d $halfRes -cobound 1 "$rightPeakFile" "$tmpFile3" -prefix "$tmpFile5" 2> /dev/null`;
	`rm "$tmpFile3"`;

	open IN1, "$tmpFile4.coBoundBy1.txt" or die "!!! Couldn't open file: $tmpFile4.coBoundBy1.txt for reading!!!\n";
	while (<IN1>) {
		chomp;
		s/\r//g;
		my @line = split /\t/;
		if (exists($ints->{$line[0]})) {
			...
			if (!exists($ints->{$line[0]}->{'md1'})) {
				$ints->{$line[0]}->{'md1'} = $dir;
			}
		}
	}
	close IN1;
	open IN1, "$tmpFile5.coBoundBy1.txt" or die "!!! Couldn't open file: $tmpFile5.coBoundBy1.txt for reading!!!\n";
	while (<IN1>) {
		chomp;
		s/\r//g;
		my @line = split /\t/;
		if (exists($ints->{$line[0]})) {
			...
			if (!exists($ints->{$line[0]}->{'md2'})) {
				$ints->{$line[0]}->{'md2'} = $dir;
			}
		}
	}
	close IN1;
	`rm "$tmpFile4.coBoundBy1.txt" "$tmpFile4.coBoundBy0.txt" "$tmpFile5.coBoundBy0.txt" "$tmpFile5.coBoundBy1.txt"`;
	my %dirStats = ();
	my $total = 0;
	foreach(values %$ints) {
		my $int = $_;
	...
	open OUT, ">$outputDirectory/motifDirection.txt";
	print OUT "Code (_='NA',*=bothdirections)\tFraction\tCount\n";
	print STDERR "Code (_='NA',*=bothdirections)\tFraction\tCount\n";
	my @codes = sort {$dirStats{$b} <=> $dirStats{$a}} keys %dirStats;
	foreach(@codes) {
		my $c = $dirStats{$_};
		my $r = $c/$total;
		print OUT "$_\t$r\t$c\n";
...
sub getPeakFileEnrichment {
	...
	for (my $i=0;$i<@peakFiles;$i++) {
		...
		#print STDERR "`mergePeaks -d $halfRes $intPeakFile $resPeakFiles[$i] -matrix $tmpFile5 $gsizeOpt > /dev/null 2> /dev/null`;\n";
		#exit;
