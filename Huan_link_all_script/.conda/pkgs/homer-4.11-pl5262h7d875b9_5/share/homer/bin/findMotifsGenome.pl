...
sub printCMD {
	...
	print STDERR "\t\t\tremoves background positions overlapping with target positions unless -keepOverlappingBg is used\n";
	...
	print STDERR "\t\t-seqlogo (use weblogo/seqlogo/ghostscript to generate logos, default uses SVG now)\n";
	...
	#print STDERR "\t\t-repeats (normalize background to repeat content, i.e. analyzing H3K9me3 peaks)\n";
...
sub parseCMDLine {
	...
	my $cmd = {posfile=>$ARGV[0], fg=>'', bg=>'', 
				...
				preparsedDir=>'/',repeats=>0,seqlogo=>"",removeOverlappingBg=>1
			};
	print STDERR "\n";
	...
	for (my $i=3;$i<@$argv;$i++) { 
		if ($ARGV[$i] eq '-bg') {
			...
			$cmd->{'repeats'} = 1;
		} elsif ($ARGV[$i] eq '-keepOverlappingBg' ) {
			$cmd->{'removeOverlappingBg'}=0;
...
if ($cmd->{'find'} eq '') {
	...
	if ($executeFlag == 1) {
		if ($cmd->{'bg'} ne '' || $cmd->{'local'} > 0) {
			...
			if ($cmd->{'cgweight'} == 1 && $cmd->{'tssweight'} == 1) { 
				...
				print STDERR "\n\tChoosing background that matches in repeat element content...\n";
	...
	if ($cmd->{'nomotif'} == 1) {
		...
		if ($cmd->{'checkFlag'} == 1 && $cmd->{'nomotif'} == 0) {
			...
			`compareMotifs.pl "$cmd->{'output'}/homerMotifs.all.motifs" "$cmd->{'output'}/" -reduceThresh $reduceThresh -matchThresh $matchThresh -known "$cmd->{'mcheck'}" $cmd->{'bits'} $cmd->{'nofacts'} -cpu $cmd->{'cpus'} $rnaFlag $cmd->{'seqlogo'}`;
		}
		`rm -f "$tmpFile"`;
	}
