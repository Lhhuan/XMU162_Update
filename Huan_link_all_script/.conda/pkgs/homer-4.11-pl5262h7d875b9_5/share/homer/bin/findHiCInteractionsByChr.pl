...
sub printCMD() {
    print STDERR "\n\tAlso consider findTADsAndLoops.pl (newer version of Hi-C loop/interaction calling)\n";
	...
	print STDERR "\tThis program finds interactions that are enriched relative to expected distance and\n";
	print STDERR "\tHi-C coverage per sets of regions\n";
    print STDERR "\n\tOptions:\n";
...
for (my $i=1;$i<@ARGV;$i++) {
    if ($ARGV[$i] eq '-res') {
        $res = $ARGV[++$i];
		$resSet = 1;
    } elsif ($ARGV[$i] eq '-superRes' || $ARGV[$i] eq '-window') {
        $superRes = $ARGV[++$i];
		$superResSet = 1;
    } elsif ($ARGV[$i] eq '-ped') {
        $ped = $ARGV[++$i];
...
foreach(@chrs) {
	...
	if ($pid == 0) {
		...
		`analyzeHiC "$directory" -res $res -window $superRes -chr $chr -distNorm -std $stdFilter -min $minFilter -nomatrix -pvalue $pvalue -zscore $zscore -minDist $minDist -maxDist $maxDist -interactions "$outFile" $ped -center`;
		exit(0);
	}
	if ($cpus >= $maxCPUs) {
		my $id = wait();
