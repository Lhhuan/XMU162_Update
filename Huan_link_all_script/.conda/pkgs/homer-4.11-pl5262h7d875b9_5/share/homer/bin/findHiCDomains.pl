...
if ($fileFlag==0) {
	...
	foreach(@chrs) {
		...
		if ($pid == 0) {
			...
			`analyzeHiC "$directory" -res $res -window $superRes -boundary $domainDist -maxDist $domainDist -minDist $minDist $nolog $normType -nomatrix -chr $chr -std $stdFilter -min $minFilter > "$chrTmpFile" 2> /dev/null`;
