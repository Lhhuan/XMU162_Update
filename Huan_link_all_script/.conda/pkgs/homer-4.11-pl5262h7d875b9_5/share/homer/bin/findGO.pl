...
sub printCMD {
	...
	print STDERR "\t\t-customID (don't convert IDs)\n";
	print STDERR "\t\t-ontology <file.genes> [file2.genes] ... (custom ontologies)\n";
	print STDERR "\n";
	exit;
}
if (@ARGV < 3) {
...
if ($customIDflag) {
	`cp "$inputIDfile" "$tmpFileFg"`;
	if ($bgIDfile ne '') {
		`cp "$bgIDfile" "$tmpFileBg"`;
	} else {
		if (-f "$promoterDir/$org.base") {
			`cp "$promoterDir/$org.base" "$tmpFileBg"`;
		}
	}
} elsif ($humanFlag == 0) {
...
while (<IN>) {
	...
	next if (/^#/);
	my @line = split /\t/;
	my $go = {name=>$line[0],genes=>$line[1],output=>$line[2],desc=>$line[3],url=>$line[4],linkName=>$line[5],custom=>0};
	push(@ontologies,$go);
	
}
close IN;

foreach(@customOntologies) {
	my $n = $_;
	my $nn = $_;
	$nn =~ s/^.*\///;
	my $go = {name=>$nn,genes=>$n,output=>"$nn.txt",desc=>"$nn custom ontology",url=>"NA",linkName=>"NA",custom=>1};
	push(@ontologies,$go);
}
...
foreach(@ontologies) {
	...
	if ($pid == 0) {
		#child process
		my $fullFile = "$goDir/$org." . $go->{'genes'};
		if ($go->{'custom'}) {
			$fullFile = $go->{'genes'};
		}
		`findGOtxt.pl "$fullFile" "$tmpFileBg" "$tmpFileFg" > "$outDir/$go->{'output'}"`;
		exit(0);
	}
	if ($cpus >= $maxCPUs) {
		wait();
...
`rm "$tmpFileFg" "$tmpFileBg"`;	

my %results = ();
for (my $i=0;$i<@ontologies;$i++) {
	my $go = $ontologies[$i];
	parseResults("$outDir/$go->{'output'}", $go->{'name'});
}
...
print MAIN "<A HREF=\"homerResults.html\">Homer <i>de novo</i> Motif Enrichment Results</A><BR/>\n";
...
sub parseResults {
	...
	my $c = 0;
	open TMP, ">$tmpFileFg";
	print TMP "TermID\tTerm\tEnrichment\tlogP\tGenes in Term\tTarget Genes in Term\tFraction of Targets in Term\tTotal Target Genes\tTotal Genes\tEntrez Gene IDs\tGene Symbols\n";
	...
	while (<IN>) {
		...
		$results{$goid} = {term=>$term, lp=>$lp, p=>$pvalue, t=>$numTerm,c=>$common,
