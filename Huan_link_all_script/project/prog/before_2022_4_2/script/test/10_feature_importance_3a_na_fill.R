...
library(ggpubr)
library(Hmisc)
dat <- read.csv("06_step1_feature_importance_3a_fill.txt",na.strings = "",sep="\t")
...
pdf("./figure/10_feature_importance_3a_na_fill_step1.pdf",width = 5,height = 6)
...
dev.off()
#--------------------step6
dat <- read.csv("06_step3_feature_importance_3a_fill.txt",na.strings = "",sep="\t")
dat$feature <-as.factor(dat$feature)
dat$feature <-gsub("_"," ",dat$feature)
dat$feature <-capitalize(dat$feature) 
pdf("./figure/10_feature_importance_3a_na_fill_step3.pdf",width = 5,height = 6)
p1 <-ggplot(data = dat, mapping = aes(x = reorder(feature, weight), y = weight*100)) + geom_bar(stat = 'identity', fill = "#5eaaa8", width=0.5)+
  coord_flip()+p_theme+labs(y="Feature importance(%)",x=" ",title="Step3") +scale_y_continuous(expand=c(0,0))+
  theme(axis.text.y = element_text(color="black"),
    axis.text.x = element_text(color="black"))
print(p1)
dev.off()