...
library(Hmisc)
library(reshape2)
load("01_refine_grade_add_age.Rdata")
...
org_f_t <- org_grade %>%select(grade,age,Ki.67,LN_num,LDH,extend_num,spleen,BM,Lym_Mono,B2mg,stage,SPD,SUVmax)
...
age_more <-filter(org_f_t,age>65)%>%group_by(grade)%>%summarise(count=n())%>%as.data.frame()
age_more$class <-"age>65"
...
# suv
# spd
aaa <-bind_rows(grade_num,age_more,ki67_m,LN_num_m,extend_num_m,stage1_m,stage2_m,LDH_m,spleen_m,BM_m,Lym_Mono_m,B2mg_m,suv_m,)
...
ccc$Perc_0 <- ccc$Num_0/ccc[5,1]*100
ccc$Perc_3a <- ccc$Num_3a/ccc[5,2]*100
ccc$Perc_3b <- ccc$Num_3b/ccc[5,3]*100
ddd <-ccc[c(5,1:4,6:nrow(ccc)),c(1,4,2,5,3,6)]
write.table(ddd,"02_feature_distribution_in_grade_0_3a_3b.txt",row.names = T,col.names = T,quote = F,sep = "\t")
ddd_r <-ddd
ddd_0_3a <-ddd[,1:4]
i=2
chi_3a_0 <-function(i){
...
f_0_3a <-f_0_3a[,c(5,1:4,6:ncol(f_0_3a))]
...
eee$perc_0_3b <- eee$num_0_3b/eee[1,"num_0_3b"]
...
f_3a_other <-f_3a_other[,c(5,1:4,6:ncol(f_3a_other))]
