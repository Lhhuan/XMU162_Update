...
library(Hmisc)
library(reshape2)
load("01_add_age_raw_pfs_os_filter_grade_mergrPod.Rdata")
org_grade <-dat
#--------------------
...
chi_t <-function(i){
  ...
  names(x)<-NULL
  X_result =data.frame(P_value=tab_Xsqtest$p.value,Chi_squared=round(x,2),class=rownames(ddd)[i])
  return(X_result)
}
...
pp_r<-unlist(pp)
f_chi$P_star <- pp_r
ddd_3a$class <-rownames(ddd_3a)
f3a <-left_join(ddd_3a,f_chi,by="class")
f3a <-f3a[,c(5,1:4,6,8,7)]
write.table(f3a,"02_feature_0_3a_chi.txt",col.names = T,row.names = F,quote=F,sep = "\t")
...
f3a <-left_join(ddd_3a,f_chi,by="class")
f3a <-f3a[,c(5,1:4,6,8,7)]
write.table(f3a,"02_feature_other_3a_chi.txt",col.names = T,row.names = F,quote=F,sep = "\t")
#---------------------------------3a,3b
# fff <-ddd_r
ddd_3a <- ddd_r[,c(5,6,3,4)]
ddd<-ddd_r[,c(5,6,3,4)]
#----------------
chi_re <-lapply(c(2:nrow(ddd)),chi_t)
f_chi <-do.call(rbind,chi_re)
...
ddd_3a$class <-rownames(ddd_3a)
f3a <-left_join(ddd_3a,f_chi,by="class")
f3a <-f3a[,c(5,3,4,1,2,6,8,7)]
write.table(f3a,"02_feature_3b_3a_chi.txt",col.names = T,row.names = F,quote=F,sep = "\t")
