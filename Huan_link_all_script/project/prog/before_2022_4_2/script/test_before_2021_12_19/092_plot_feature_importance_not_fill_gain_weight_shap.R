...
library(ggpubr)
library(Hmisc)
dat <- read.csv("091_not_fill_feature_importance_weight.txt",na.strings = "",sep="\t")
...
pdf("./figure/092_not_fill_feature_importance_weight.pdf",width = 4,height = 4)
...
write.table(weight_a,"092_feature_ratio.txt",col.names=T,row.names=F,quote=F,sep="\t")
#
#--------------------gain
dat <- read.csv("091_not_fill_feature_importance_gain.txt",na.strings = "",sep="\t")
dat$Feature <-as.factor(dat$Feature)
dat$Feature <-gsub("_"," ",dat$Feature)
dat$Feature <-gsub("Lym Mono","Lym/Mono",dat$Feature)
dat$Feature <-capitalize(dat$Feature) 

pdf("./figure/092_not_fill_feature_importance_gain.pdf",width = 4,height = 4)
p1 <-ggplot(data = dat, mapping = aes(x = reorder(Feature, Feature_importance), y = Feature_importance)) + geom_bar(stat = 'identity', fill = "#5eaaa8", width=0.5)+
...
#---------------------shap
dat <- read.csv("091_feature_importance_shap_class.txt",na.strings = "",sep="\t")
dat <-abs(dat)
...
pdf("./figure/092_not_fill_feature_importance_shap.pdf",width = 4,height = 4)
