...
library("survival")
library("survminer")
# save(test_set_number,train_set_number,file= "./figure/test/adjust3_sig_test_train1_3.Rdata")
setwd("/home/huanhuan/project/prog/script/hh/")
dat <- read.csv("/home/huanhuan/project/prog/output/01_add_age_raw_pfs_os_filter_grade_mergrPod_3A.txt",sep="\t")
# load("/home/huanhuan/project/prog/output/figure/test/adjust3_sig_test_train1_2.Rdata")
...
fit <- survfit(Surv(pfs_month_new, pro_status) ~ FLIPI1_re, data=test)
pdf("./figure/test/069_pre_survival_3a_pod_total_new_sum_pfs_FLIPI1_01.pdf")
p1 <- ggsurvplot(fit,
                  pval = TRUE,
                 legend.title="Pod total new",
                 title="3a: Pfs",
...
fit <- survfit(Surv(os_month_new, dead) ~ FLIPI1_re, data=test)
pdf("./figure/test/069_pre_survival_3a_pod_total_new_sum_os_FLIPI1_01.pdf")
p1 <- ggsurvplot(fit,
                  pval = TRUE,
                 legend.title="Pod total new",
                 title="3a: Os",
...
fit <- survfit(Surv(pfs_month_new, pro_status) ~ FLIPI2_re, data=test)
pdf("./figure/test/069_pre_survival_3a_pod_total_new_sum_pfs_FLIPI2_01.pdf")
p1 <- ggsurvplot(fit,
                  pval = TRUE,
                 legend.title="Pod total new",
                 title="3a: Pfs",
...
fit <- survfit(Surv(os_month_new, dead) ~ FLIPI2_re, data=test)
pdf("./figure/test/069_pre_survival_3a_pod_total_new_sum_os_FLIPI2_01.pdf")
p1 <- ggsurvplot(fit,
                  pval = TRUE,
                 legend.title="Pod total new",
                 title="3a: Os",
