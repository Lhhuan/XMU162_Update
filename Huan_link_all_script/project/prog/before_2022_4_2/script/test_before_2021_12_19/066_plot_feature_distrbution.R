...
pod_total3_num  <-table(dat$pod_total3)%>%as.data.frame()
pdf("./figure/066_distrbution_pod_total3.pdf",width = 5,height = 5)
p1 <-ggplot(data = pod_total3_num , mapping = aes(x=Var1,y=Freq)) + geom_bar(stat = 'identity', fill = "#5eaaa8", width=0.5)+
  p_theme+labs(y="Number of samples",x="Pod total3") +scale_y_continuous(expand=c(0,0))+
  theme(axis.text.y = element_text(color="black",size=8),
    axis.text.x = element_text(color="black",size=8))
print(p1)
dev.off()

colnames(pod_total3_num) <-c("pod_total3","number")
pod_total3_num$ratio <-pod_total3_num$number/sum(pod_total3_num$number)
...
write.table(cutoff,"066_cutoff.txt",col.names=T,row.names=F,quote=F,sep="\t")
#---------------------
dat$B2mg_c <-dat$B2mg
dat$B2mg_c[dat$B2mg_c <=3.41]=0
dat$B2mg_c[dat$B2mg_c >3.41 &dat$B2mg_c <=3.84]=3
dat$B2mg_c[dat$B2mg_c >3.84]=4

dat$LN_num_c <-dat$LN_num
dat$LN_num_c[dat$LN_num_c <=10.00]=0
dat$LN_num_c[dat$LN_num_c >10.00 &dat$LN_num_c <=11.00]=3
dat$LN_num_c[dat$LN_num_c >11.00]=4

dat$LDH_c <-dat$LDH
dat$LDH_c[dat$LDH_c <=274.00]=0
dat$LDH_c[dat$LDH_c >274.00 &dat$LDH_c <=335.00]=3
dat$LDH_c[dat$LDH_c >335.00]=4

dat$age_raw_c <-dat$age_raw
dat$age_raw_c[dat$age_raw_c <=63.00]=0
dat$age_raw_c[dat$age_raw_c >63.00 &dat$age_raw_c <=66.00]=3
dat$age_raw_c[dat$age_raw_c >66.00]=4

dat$Lym_Mono_c <-dat$Lym_Mono
dat$Lym_Mono_c[dat$Lym_Mono_c <=5.50]=0
dat$Lym_Mono_c[dat$Lym_Mono_c >5.50 &dat$Lym_Mono_c <=6.60]=3
dat$Lym_Mono_c[dat$Lym_Mono_c >6.60]=4

dat$SUVmax_c <-dat$SUVmax
dat$SUVmax_c[dat$SUVmax_c <=18.00]=0
dat$SUVmax_c[dat$SUVmax_c >18.00 &dat$SUVmax_c <=20.70]=3
dat$SUVmax_c[dat$SUVmax_c >20.70]=4


dat$SPD_c <-dat$SPD
dat$SPD_c[dat$SPD_c <=22.64]=0
dat$SPD_c[dat$SPD_c >22.64 &dat$SPD_c <=33.06]=3
dat$SPD_c[dat$SPD_c >33.06]=4

dat$Ki.67_c <-dat$Ki.67
dat$Ki.67_c[dat$Ki.67_c <=70.00]=0
dat$Ki.67_c[dat$Ki.67_c >70.00 &dat$Ki.67_c <=75.00]=3
dat$Ki.67_c[dat$Ki.67_c >75.00]=4

dat$HGB_c <-dat$HGB
dat$HGB_c[dat$HGB_c <=70.00]=4
dat$HGB_c[dat$HGB_c >70.00 &dat$HGB_c <=75.00]=3
dat$HGB_c[dat$HGB_c >75.00]=3
...
dat$Bsym_c_f <- dat$Bsym *1.5
dat$BM_c_f<-dat$BM*1.5
#-------------
dat_s<-dat[,c("B2mg_c_f","LN_num_c_f","LDH_c_f","age_raw_c_f","Lym_Mono_c_f","HGB_c_f","Ki.67_c_f","SPD_c_f","SUVmax_c_f","Bsym_c_f","BM_c_f")]


dat$predict_total3 <- base::rowSums(dat_s, na.rm = TRUE)
dat$predict_total3_f <-dat$predict_total3
dat1 <-dat[order(dat$predict_total3_f),]
dat1[1:round(nrow(dat)*0.80633803),"predict_total3_f"]=0

dat1[round(nrow(dat)*0.80633803):round(nrow(dat)*0.8755869),"predict_total3_f"]=3
dat1[round(nrow(dat)*0.8755869):nrow(dat),"predict_total3_f"]=4
...
fit <- survfit(Surv(pfs_month_new, pro_status) ~ predict_total3_f, data=dat1)
pdf("./figure/066_pre_survival_3a_pod_total3_sum_pfs.pdf")
p1 <- ggsurvplot(fit,
                  pval = TRUE,
                 legend.title="Pod total3",
                 title="3a: Pfs",
                 xlab = " Time (Months)")
print(p1) 
dev.off()
#------------------------------
fit <- survfit(Surv(os_month_new, dead) ~ predict_total3_f, data=dat1)
pdf("./figure/066_pre_survival_3a_pod_total3_sum_Os.pdf")
p1 <- ggsurvplot(fit,
                  pval = TRUE,
                 legend.title="Pod total3",
                 title="3a: Os",
