...
write.table(aa,"03_fill_univariate_cox_result_not_fill_all.txt" ,quote=F,sep="\t",row.names=FALSE)
...
pdf("./figure/03_univariate_cox_not_fill_na_all.pdf",width=5,height=5)
...
res.cox <- coxph(Surv(os_month_new, dead) ~ CR+Rmaintain+HGB+B2mg+LDH+age_raw+SUVmax+extend_num+stage+site0+HGB0+Bsym+LN6+ECOG+interm_res+end_res+b2mg_LDH+X150b2mg_ldh+progress+trans+LDH0, data =  prog)
x <- summary(res.cox)
...
aa$var <-gsub("_"," ",aa$var)
aa$var <-capitalize(aa$var)
pdf("./figure/03_Multivariable_cox_not_fill_na_all.pdf",width=5,height=5)
p1<-ggplot(data=aa, aes(x=reorder(var,X=HR),y=HR, ymin=low , ymax=high))
  p1<-p1+geom_pointrange(size = 0.3) 
  p1<-p1+coord_flip()# +  # flip coordinates (puts labels on y axis)
  p1<-p1+labs(y="Hazard ratio",x="",title="Multivariable analysis") 
  p1 <-p1+p_theme+theme(axis.text.x = element_text(color = "black"),axis.text.y = element_text(color = "black"),plot.title = element_text(hjust = 0.5))
  p1 <-p1+geom_text(aes(x=var,y=100.5,label = significant)) +
      geom_hline(yintercept=1,colour="#c00024",linetype=3)
print(p1)
dev.off()


write.table(aa, "03_multivariate_cox_result_not_fill_all.txt",quote=F,sep="\t")
