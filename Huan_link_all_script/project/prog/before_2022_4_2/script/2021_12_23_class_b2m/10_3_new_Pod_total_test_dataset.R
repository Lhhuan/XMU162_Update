...
#-----------------------------------------------adjust1
test$Lym_Mono_c_f <-test$Lym_Mono_c*29.35
test$LDH_c_f <- test$LDH_c * 15.92
test$HGB_c_f<- test$HGB_c *12.94
test$SUVmax_c_f<- test$SUVmax_c *9.95
test$ECOG_c_f<- test$ECOG_c *8.46
test$LN_num_c_f <-test$LN_num_c *7.96
test$extend_num_c_f <-test$extend_num_c *5.97
test$SPD_c_f<- test$SPD_c *3.98
test$Bsym_c_f <- test$Bsym_c*3.98
test$BM_extend_c_f<-test$BM_extend_c*1.49

test$sum_score<-base::rowSums(test[,c("Lym_Mono_c_f","LDH_c_f","HGB_c_f","SUVmax_c_f","ECOG_c_f","LN_num_c_f","extend_num_c_f","SPD_c_f","Bsym_c_f","Bsym_c_f")])
# test[is.na(test$sum_score),]
test$adjust1 <- test$sum_score
# adjust1 <-test
#-----------------------------------------------adjust2
test$Lym_Mono_c_f <-test$Lym_Mono_c*12
test$LDH_c_f <- test$LDH_c * 6
test$HGB_c_f<- test$HGB_c *5
test$SUVmax_c_f<- test$SUVmax_c *4
test$ECOG_c_f<- test$ECOG_c *3
test$LN_num_c_f <-test$LN_num_c *3
test$extend_num_c_f <-test$extend_num_c *2
...
#-----------------------------------------------adjust3
test$Lym_Mono_c_f <-test$Lym_Mono_c*6
test$LDH_c_f <- test$LDH_c * 3
test$HGB_c_f<- test$HGB_c *3
test$SUVmax_c_f<- test$SUVmax_c *2
test$ECOG_c_f<- test$ECOG_c *2
...
p_theme<-theme(panel.grid =element_blank())+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
                                                  ...
                                                  axis.line = element_line(colour = "black"),plot.title = element_text(hjust = 0.5),legend.title=element_blank(),legend.text=element_text(size=9))

mycolor <-c("#673AB7","#9C27B0","#E53935","#827717","#1B5E20","#006064","#01579B")
#--------------------------------------------------------------------------8
library(plotROC)
longtest <-melt_roc(test,"new_pod_total",c("FLIPI1_count","FLIPI2_count","primapi_re_n","X150b2mg_ldh","adjust2","adjust3","adjust4"))
...
p <-ggplot(longtest,aes(d=D,m=M,color=name))+geom_roc(n.cuts = 0,show.legend = TRUE,labels=FALSE) + scale_color_manual(values=mycolor)+#,color=name aes(color=mycolor)
...
ggsave("./figure/10_3_cutoff1_roc7.png",p1,dpi=300,width=7,height=5.8)

#------------------------------------------------------
#--------------------------------------------------------------------------adjust3
TP <-length(test$new_pod_total[test$adjust3>=8&test$new_pod_total==1])
FP <-length(test$new_pod_total[test$adjust3>=8&test$new_pod_total==0])
FN <-length(test$new_pod_total[test$adjust3<8&test$new_pod_total==1])
TN <-length(test$new_pod_total[test$adjust3<8&test$new_pod_total==0])
TPR = TP / (TP+FN)
FPR = FP / (FP + TN)
...
p1 <- p+annotate("text",x=0.85,y=0.15,
  label=paste(names(auc)[1]," AUC: ",round(auc[1],3),"\n",
              ...
              names(auc)[5]," AUC: ",round(auc[5],3),"\n"),
              size=3.5)
p1 <-p1 +annotate("text",x=0.41,y=0.89,label= "8(0.257,0.909)" )+geom_point(x=FPR,y=TPR ,size=4,color="#E53935" )
ggsave("./figure/10_3_cutoff1_roc_adjust3.png",p1,dpi=300,width=7,height=5.8)
...
ggsave("./figure/093_pfs_FLIPI1.png",p1$plot,dpi=300,height=5.2,width=5)
# dev.off()
...
ggsave("./figure/093_os_FLIPI1.png",p1$plot,dpi=300,height=5.2,width=5)
#--------------------------------------------------------------------------------2
...
# print(p1$plot) 
# dev.off()
ggsave("./figure/093_pfs_FLIPI2.png",p1$plot,dpi=300,height=5.2,width=5)
...
ggsave("./figure/093_os_FLIPI2.png",p1$plot,dpi=300,height=5.2,width=5)
#-------------------------------------------------prima_pi
...
ggsave("./figure/093_pfs_PRIMAPI.png",p1$plot,dpi=300,height=5.2,width=5)
...
ggsave("./figure/093_os_PRIMAPI.png",p1$plot,dpi=300,height=5.2,width=5)
#--------------------------------------------------------------------------LDH + β2M
...
ggsave("./figure/093_pfs_X150b2mg_ldh.png",p1$plot,dpi=300,height=5.2,width=5)
...
ggsave("./figure/093_os_X150b2mg_ldh.png",p1$plot,dpi=300,height=5.2,width=5)
...
ggsave("./figure/10_3_pod24_pfs_cutoff1_adjust3.png",p1$plot,dpi=300,height=5.2,width=5)
#------------------------------
...
ggsave("./figure/10_3_pod24_Os_cutoff1_adjust3.png",p1$plot,dpi=300,height=5.2,width=5)
#---------------------------------------------------------------------------------------------ori
