...
dat$grade[grep("低级�?",dat$grade)] <- 0
dat$grade[grep("转化",dat$grade)] <- "3b"
dat$grade[grep("3B",dat$grade)] <- "3b"
dat$grade[grep("高级�?",dat$grade)] <- "3b"
dat$grade[grep("滤泡弥漫混合�?",dat$grade)] <- "3b"
dat$grade[grep("滤泡弥漫�?",dat$grade)] <- "3b"
...
dat$Ki.67[grep("10�??20�??",dat$Ki.67)] <- 15
dat$Ki.67[grep("10�??30�??",dat$Ki.67)] <- 20
...
dat$Ki.67[grep("5�??10�??",dat$Ki.67)] <- 7.5
...
dat$LN_num[grep("肠系�?? 腹腔及腹膜后多发肿大淋巴结，FDG代谢异常增高",dat$LN_num)] <- 25
...
dat$dead[grep("NA",dat$dead)]<- 0
dat$dead<-as.numeric(dat$dead)

dat$pro_time<-as.Date(dat$pro_time)
dat$pro_status[grep("NA",dat$pro_status)]<- 0
...
dat$BM_extend[grep("否|�??",dat$extend_site_out_node)]<- 0
dat$BM_extend[grep("骨髓",dat$BM_extend)]<- 0
dat$BM_extend[grep("�??",dat$BM_extend)]<- 1
