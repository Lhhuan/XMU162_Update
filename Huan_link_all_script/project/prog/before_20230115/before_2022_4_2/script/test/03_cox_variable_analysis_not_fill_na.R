...
pdf("./figure/univariate_cox_not_fill_na.pdf",width=5,height=5)
p1<-ggplot(data=aa, aes(x=reorder(var,X=HR),y=HR, ymin=HR_confint_lower , ymax=HR_confint_upper))
  ...
  p1<-p1+labs(y="Hazard ratioHazard ratio",x="",title="Univariable analysis") 
...
res.cox <- coxph(Surv(os_month_new, dead) ~ gender + Ki.67 + stage + Bsym + LN_num + site0 + extend + BM + spleen + extend_num + BM_extend + LN6 + SUVmax + SPD + X150b2mg_ldh + b2mg_LDH + ECOG + B2mg + LDH + LDH0 + WBC + HGB + HGB0 + PLT + Mono + Lym + interm_res + end_res + CR + Rmaintain + progress + trans + relapes + relapse_res + age_raw + age_raw_60 + ki67_20 + LN_num_6 + extend_num_0 + stage_III_IV + stage_I_II + SUVmax_2 + LDH_300 + Lym_Mono_10 + B2mg_3.4 + SPD_0, data =  prog)
x <- summary(res.cox)
