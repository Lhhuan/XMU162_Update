...
write.table(prog,"01_add_age_raw_pfs_os_filter_grade_mergrPod_3A.txt",quote=F,sep="\t",row.names=FALSE)
...
pdf("./figure/03_ori_survival_3a_pod_total_pfs.pdf")
p1 <- ggsurvplot(fit,
...
write.table(aa,"03_fill_univariate_cox_result_not_fill_3a.txt" ,quote=F,sep="\t",row.names=FALSE)
...
pdf("./figure/03_univariate_cox_not_fill_na_3a.pdf",width=5,height=5)
p1<-ggplot(data=aa, aes(x=reorder(var,X=HR),y=HR, ymin=HR_confint_lower , ymax=HR_confint_upper))
  ...
  p1 <-p1+geom_text(aes(x=var,y=9.5,label = significant)) +
...
res.cox <- coxph(Surv(os_month_new, dead) ~ BM+X150b2mg_ldh+b2mg_LDH+ECOG+LDH+LDH0+HGB+interm_res+end_res+CR+progress+age_raw, data =  prog)
x <- summary(res.cox)
...
aa$var <-gsub("_"," ",aa$var)
aa$var <-capitalize(aa$var)
pdf("./figure/03_Multivariable_cox_not_fill_na_3a.pdf",width=5,height=5)
p1<-ggplot(data=aa, aes(x=reorder(var,X=HR),y=HR, ymin=low , ymax=high))
  p1<-p1+geom_pointrange(size = 0.3) 
  p1<-p1+coord_flip()# +  # flip coordinates (puts labels on y axis)
  p1<-p1+labs(y="Hazard ratio",x="",title="Multivariable analysis") 
  p1 <-p1+p_theme+theme(axis.text.x = element_text(color = "black"),axis.text.y = element_text(color = "black"),plot.title = element_text(hjust = 0.5))
  p1 <-p1+geom_text(aes(x=var,y=10.5,label = significant)) +
      geom_hline(yintercept=1,colour="#c00024",linetype=3)
print(p1)
dev.off()


write.table(aa, "03_multivariate_cox_result_not_fill_3a.txt",quote=F,sep="\t")
