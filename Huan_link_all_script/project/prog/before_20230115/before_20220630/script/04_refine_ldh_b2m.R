...
library(ggpubr)
library(Hmisc)
data <- read.csv("D:\\Huan_R\\prog\\before_2022_4_2\\data\\data.csv",na.strings = "")
colnames(data)[78:79] <- c("pro_time","pro_status")
load("D:\\Huan_R\\prog\\before_2022_4_2\\data\\01_add_age_raw_pfs_os_filter_grade.Rdata")
dat1 <-left_join(dat,data[,c("No","hospital")],by="No")
unique(dat1$hospital)
dat1$hospital[is.na(dat1$hospital)]="中山大学肿瘤防治中心"
peking <-read.csv("D:\\Huan_R\\prog\\before_2022_4_2\\data\\B2M_LDH_extract\\peking.csv",na.strings = "")
peking <-peking[,c("No","LDHg","b2MG2")]
ruijin <-read.csv("D:\\Huan_R\\prog\\before_2022_4_2\\data\\B2M_LDH_extract\\ruijin.csv",na.strings = "")
colnames(ruijin)=ruijin[1,]
ruijin<-ruijin[-1,]
ruijin <-ruijin[,c("No","LDH","B2-MG")]
tianzhong <-read.csv("D:\\Huan_R\\prog\\before_2022_4_2\\data\\B2M_LDH_extract\\tianzhong.csv",na.strings = "")
