...
features = c('stage','Bsym','LN6','BM','spleen','extend_num','ECOG','B2MG_re0_train','LDH_re0_train',"pfs_month_new","pro_status","grade","No")
# aaa <-dat_usedf[which(is.na(dat_usedf$pfs_month_new)),]

dat_used <-left_join(dat_filter[,features],dat,by="No")
colnames(dat_used)[c(1:10,14)] <-c('Stage','Bsym','LoDLIN_6cm','BM','Spleen','Extra_sites','ECOG','B2MG','LDH','Time','Grade')
...
library(regplot)
regplot(m.crr,plots=c("density","spikes"), observation=FALSE,points=TRUE,dencol="#84B1ED",spkcol="#84B1ED",
        failtime = c(12, 24),title="Relapse: classification")
