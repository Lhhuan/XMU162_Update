...
dat$sum_score<-base::rowSums(dat[,c("LDH_c_f","B2M_c_f","HGB_c_f","LN_num_c_f","ECOG_c_f","Lym_Mono_c_f","SUVmax_c_f")])
# dat[is.na($sum_score),]
dat$adjust3 <- dat$sum_score
#-----------------------------------------------adjust4
...
dat$pod_total_merge12_34_c <-dat$pod_total_merge12_34
dat$pod_total_merge12_34_c[grep("0",dat$pod_total_merge12_34_c)] <- "Low"
dat$pod_total_merge12_34_c[grep("1",dat$pod_total_merge12_34_c)] <- "Intermediate"
dat$pod_total_merge12_34_c[grep("2",dat$pod_total_merge12_34_c)] <- "High"

dat$b2mg_ldh1.5s_c <- dat$b2mg_ldh1.5s
dat$b2mg_ldh1.5s_c[grep("0",dat$b2mg_ldh1.5s_c)] <- "Low"
dat$b2mg_ldh1.5s_c[grep("1",dat$b2mg_ldh1.5s_c)] <- "Intermediate"
dat$b2mg_ldh1.5s_c[grep("2",dat$b2mg_ldh1.5s_c)] <- "High"

dat$adjust3_c <-NA
dat$adjust3_c [dat$adjust3  <=3]="Low"
dat$adjust3_c [dat$adjust3>3 & dat$adjust3<5]="Intermediate"
dat$adjust3_c [dat$adjust3>=5]="High"

dat$adjust4_c <-NA
dat$adjust4_c [dat$adjust4  <=3]="Low"
dat$adjust4_c [dat$adjust4>3 & dat$adjust4<5]="Intermediate"
dat$adjust4_c [dat$adjust4>=5]="High"

train <-dat[train_set_number,]
train1 <-melt(train[,c("No","pod_total_merge12_34_c","pod_total_merge12_34","adjust3","adjust4")], id=c("No","pod_total_merge12_34_c","pod_total_merge12_34"))
train1$variable <-gsub("primapi_re_n","PRIMA-PI",train1$variable)
train1$variable <-gsub("b2mg_ldh1.5s","LDH+B2mg",train1$variable)
p1 <- ggplot(train1, aes(x = variable, y = value, fill = pod_total_merge12_34_c)) +
  geom_boxplot() +
  scale_y_continuous(name = "Score") +
  scale_x_discrete( name = "Models")

ggsave("./figure/10_3_distribution_of_train_score_Ours_model.png",p1,dpi=300,width=7,height=5.8)

#--------------------------------------
#-----prepare for mulitclass ROC
test <-dat[test_set_number,]
true_label <- dummies::dummy(test$pod_total_merge12_34_c, sep = ".")
colnames(true_label) <- gsub(".*?\\.", "", colnames(true_label))
colnames(true_label) <- paste(colnames(true_label), "_true")
#------adjust3
adjust3_pred <- dummies::dummy(test$adjust3_c, sep = ".")
colnames(adjust3_pred) <- gsub(".*?\\.", "", colnames(adjust3_pred))
colnames(adjust3_pred) <- paste(colnames(adjust3_pred), "_pred_adjust3")
#----------adjust4
adjust4_pred <- dummies::dummy(test$adjust4_c, sep = ".")
colnames(adjust4_pred) <- gsub(".*?\\.", "", colnames(adjust4_pred))
colnames(adjust4_pred) <- paste(colnames(adjust4_pred), "_pred_adjust4")
#---------
FLIPI1_pred <- dummies::dummy(test$FLIPI1_count_re, sep = ".")
colnames(FLIPI1_pred) <- gsub(".*?\\.", "", colnames(FLIPI1_pred))
colnames(FLIPI1_pred) <- paste(colnames(FLIPI1_pred), "_pred_FLIPI1")

FLIPI2_pred <- dummies::dummy(test$FLIPI2_count_re, sep = ".")
colnames(FLIPI2_pred) <- gsub(".*?\\.", "", colnames(FLIPI2_pred))
colnames(FLIPI2_pred) <- paste(colnames(FLIPI2_pred), "_pred_FLIPI2")

primapi_pred <- dummies::dummy(test$primapi_re, sep = ".")
colnames(primapi_pred) <- gsub(".*?\\.", "", colnames(primapi_pred))
colnames(primapi_pred) <- paste(colnames(primapi_pred), "_pred_PRIMA-PI")

b2mg_ldh1.5s_pred <- dummies::dummy(test$b2mg_ldh1.5s_c, sep = ".")
colnames(b2mg_ldh1.5s_pred) <- gsub(".*?\\.", "", colnames(b2mg_ldh1.5s_pred))
colnames(b2mg_ldh1.5s_pred) <- paste(colnames(b2mg_ldh1.5s_pred), "_pred_LDH+B2mg")

# final_df <- cbind(true_label, adjust3_pred,adjust4_pred,FLIPI1_pred,FLIPI2_pred,primapi_pred,b2mg_ldh1.5s_pred)

final_df <- cbind(true_label, adjust3_pred,adjust4_pred,FLIPI1_pred,FLIPI2_pred,primapi_pred)
# final_df <- cbind(true_label,b2mg_ldh1.5s_pred)
library(multiROC)
roc_res <- multi_roc(final_df, force_diag=T)
plot_roc_df <- plot_roc_data(roc_res)
plot_roc_df<-filter(plot_roc_df,Group=="Micro")
AUC <-unique(plot_roc_df[,c("AUC","Method")])
...
p <- ggplot(plot_roc_df, aes(x = 1-Specificity, y=Sensitivity)) +
  geom_path(aes(color = Method), size=1.5) +scale_color_manual(values=mycolor[1:5])+
  xlab("FPR")+ylab("TPR")+
  geom_segment(aes(x = 0, y = 0, xend = 1, yend = 1), 
               colour='grey', linetype = 'dotdash') +p_theme+theme_bw()

p1 <-p+ annotate("text",x=0.85,y=0.15,
  label=paste(AUC[1,2]," AUC: ",round(AUC[1,1],3),"\n",
              AUC[2,2]," AUC: ",round(AUC[2,1],3),"\n",
              AUC[3,2]," AUC: ",round(AUC[3,1],3),"\n",
              AUC[4,2]," AUC: ",round(AUC[4,1],3),"\n",
              AUC[5,2]," AUC: ",round(AUC[5,1],3),"\n"),
              size=3.5)
ggsave("./figure/10_3_roc_test.png",p1,dpi=300,width=7,height=5.8)
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------8
library(plotROC)
longtest <-melt_roc(test,"new_pod_total",c("FLIPI1_count","FLIPI2_count","primapi_re_n","b2mg_ldh1.5s","adjust5","adjust3","adjust4"))
...
#------------------------------------------------------
#--------------------------------------------------------------------------adjust5
TP <-length(test$new_pod_total[test$adjust5>=4&test$new_pod_total==1])
FP <-length(test$new_pod_total[test$adjust5>=4&test$new_pod_total==0])
FN <-length(test$new_pod_total[test$adjust5<4&test$new_pod_total==1])
TN <-length(test$new_pod_total[test$adjust5<4&test$new_pod_total==0])
TPR = TP / (TP+FN)
FPR = FP / (FP + TN)
