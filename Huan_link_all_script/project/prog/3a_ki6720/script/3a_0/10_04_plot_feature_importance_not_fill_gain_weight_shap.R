...
library(ggpubr)
library(Hmisc)
dat <- read.csv("10_03_not_fill_feature_importance_weight.txt",na.strings = "",sep="\t")
...
#
#--------------------gain
dat <- read.csv("10_03_not_fill_feature_importance_gain.txt",na.strings = "",sep="\t")
dat$Feature <-capitalize(dat$Feature)
dat$Feature <-as.factor(dat$Feature)
dat$Feature <-gsub("LN_num","Lymph nodes",dat$Feature)
dat$Feature <-gsub("Extend_num","Extra sites",dat$Feature)
...
#---------------------shap
dat <- read.csv("10_03_not_fill_feature_importance_shap_class.txt",na.strings = "",sep="\t")
dat <-abs(dat)
