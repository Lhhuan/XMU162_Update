...
# dat$follow_day <-difftime(dat$followup,dat$diagnosis,units = "days")
# dat$follow_month <-as.numeric(round(dat$follow_day/30))
# aaa <-dat[,c("diagnosis","followup","follow_month")]
# dim(filter(dat,follow_month >=60))
# dat$age_c =dat$age_s
...
dat$B2M_c_f <-dat$B2M_c*4
dat$Lym_Mono_c_f <-dat$Lym_Mono_c*4
dat$HGB_c_f<- dat$HGB_c *2
dat$LN_num_c_f <-dat$LN_num_c *2
dat$SUVmax_c_f<- dat$SUVmax_c *2
dat$BM_c_f<- dat$BM_c *1
dat$extend_num_c_f <-dat$extend_num_c *1
dat$sum_score<-base::rowSums(dat[,c("LDH_c_f","B2M_c_f","Lym_Mono_c_f","HGB_c_f","LN_num_c_f","SUVmax_c_f","BM_c_f","extend_num_c_f")])
# dat[is.na($sum_score),]
...
# p1 <-p1 +annotate("text",x=0.35,y=TPR,label= "5",size=5,color="#E53935" )+geom_point(x=FPR,y=TPR ,size=4,color="#E53935" )
  ...
  p2 <-p1 +scale_x_continuous(limits= c(0, 1), breaks= seq(0,1,0.2))+ #expand = c(0, 0)
  scale_y_continuous(limits= c(0, 1), breaks= seq(0,1,0.2)) #+theme(panel.background = element_rect(color="black",size=1.2))
ggsave("./figure/10_3_OS_dead_roc5_test_data_new.png",p2,dpi=300,width=7,height=5.8)
pdf("aaa.pdf")
print(p2)
dev.off()
...
# test$sum_score<-base::rowSums(test[,c("LDH_c_f","B2M_c_f","Lym_Mono_c_f","HGB_c_f","LN_num_c_f","SUVmax_c_f","BM_c_f","extend_num_c_f")])
...
#                 ggtheme = custom_theme()
#                 )
# ggsave("./figure/10_3_pod24_pfs_cutoff1_adjust5.png",p1$plot,dpi=300,height=5.2,width=5)
# #------------------------------
...
#                 ggtheme = custom_theme()
#                 )
# ggsave("./figure/10_3_pod24_Os_cutoff1_adjust5.png",p1$plot,dpi=300,height=5.2,width=5)
# #-----------------
# #--------------------------------------------------------------------------------------------adjust5
# cutoff_y=5
# test$adjust5_01 <-NA
