...
all <-read.csv("all_data.2022-3-21-new_utf8.csv",na.strings = "")
...
dat$pro_status[grep("NA",dat$pro_status)] <- NA

# dat<- dat%>%drop_na(pro_status,pod12,pod24,pod36,pod48)

dat<-dat %>%filter(!(is.na(pro_status) & is.na(pod12) & is.na(pod24) & is.na(pod36) & is.na(pod48)))
# dat_na <-setdiff(dat,dat1)

dat_error <-filter(dat,pro_status==1&pod48==0) #核对进展时间无误
# dat1 <-setdiff(dat,dat_error)
# dat <-filter(dat,!(No%in%dat_error$No))
#==================================================
