...
library(Hmisc)
library(reshape2)
load("/home/huanhuan/project/prog/data/04_add_tie3_refine_ldh_b2m_20230115.Rdata")
dat <-filter(dat2,grade!="3b")
dat_m0<-filter(dat,Mono==0)
dat <-setdiff(dat,dat_m0)
#-------------
dat$age_raw_c=NA
...
aaa <-dat[,c("grade","age_raw_c","stage_c","LN_num_c","extend_num_c","BM_c","spleen_c","B2mg_c","LDH_c","Lym_Mono_c","Ki.67_c","SUVmax_c","SPD_c","HGB_c","LN6_c","ECOG_c","Bsym_c")]
...
rownames(ccc)<-c("Age >60 years","β2-MG >UNL","BM","Bsym","ECOG >1","Extra sites >0","grade_num","HGB <120g/L","Ki.67 >30%","LDH >UNL","Lymph nodes >4", "LoDLIN >6cm","Lym/Mono >10","SPD >20mm","Spleen","Stage(I-II)","Stage(III-IV)","SUVmax >10")
names_c <-data.frame(term=rownames(ccc),ori_term=ccc$ori_term)
save(names_c,file="05_terms.Rdata")
ddd <-ccc[c("grade_num","Age >60 years","Stage(I-II)","Stage(III-IV)","Lymph nodes >4","LoDLIN >6cm","Extra sites >0","BM","Spleen","Bsym","β2-MG >UNL","LDH >UNL",
