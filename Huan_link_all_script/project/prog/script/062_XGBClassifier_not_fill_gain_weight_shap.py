...
im_gain_df.to_csv("../output/062_ori_not_fill_feature_importance_gain.txt",sep="\t",index=None)
#--------------------------------------
...
im_weight_df.to_csv("../output/062_ori_not_fill_feature_importance_weight.txt",sep="\t",index=None)
...
plt.savefig('../output/figure/062_not_fill_feature_shap_bar.pdf')
plt.show()


feature_shap =pd.DataFrame(shap_values.values)
feature_shap.columns =X.columns
feature_shap.to_csv("../output/062_not_fill_feature_importance_shap_class.txt",sep="\t",index=None)


#---------------
# f_gain_nor= pd.DataFrame([X.columns,model.feature_importances_]).T
# f_gain_nor.columns=['feature','gain']
# f_gain_nor = f_gain_nor.sort_values(by='gain',ascending=False)
# f_gain_nor.to_csv("../output/06_non_oncology_feature_importance_gain_normalized.txt",sep="\t",index=None)