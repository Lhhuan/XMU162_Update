...
for thresh in thresholds:
	...
	# train model
	selection_model = XGBClassifier(reg_alpha = 1e-05, 
        colsample_bytree= 0.65, 
        subsample= 0.5,
        gamma = 1.7,
        min_child_weight =3,
...
#---------------------------------step2
X = dataset.loc[:, ['gender','Ki.67','stage','Bsym','LN_num','site0','extend','BM','spleen','extend_num','BM_extend','LN6','SUVmax','SPD','b2mg_LDH','ECOG','B2mg','LDH','LDH0','WBC','HGB','HGB0','PLT','Mono','Lym','age_raw','age_raw_60','LN_num_6','extend_num_0','LDH_300','B2mg_3.4']]
y = dataset.loc[:, 'pod_total']
X_COL = X.columns
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=7)
...
#----------------------------------step3
X = dataset.loc[:, ['gender','Ki.67','stage','Bsym','LN_num','site0','extend','BM','spleen','extend_num','BM_extend','LN6','SUVmax','SPD','b2mg_LDH','ECOG','B2mg','LDH','LDH0','WBC','HGB','PLT','Mono','Lym','age_raw','LN_num_6','B2mg_3.4']]
y = dataset.loc[:, 'pod_total']
X_COL = X.columns
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=7)
...
f_weight_step3.to_csv("../output/06_step3_feature_importance_3a_not_fill.txt",sep="\t",index=None)
f_sel_step3.to_csv("../output/06_step3_feature_selection_3a_not_fill.txt",sep="\t",index=None)