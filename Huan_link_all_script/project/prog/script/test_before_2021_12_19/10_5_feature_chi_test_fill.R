...
library(Hmisc)
library(reshape2)
load("071_add_model_index_age_raw_pfs_os_filter_grade_mergrPod_all.Rdata")

dat$age_raw_c <-dat$age_s #60
dat$stage_c <-dat$stage_s #1-2,3-4
dat$LN_num_c <-dat$LN_num_s  #4
...
dat$B2mg_c[is.na(dat$B2mg_c)]=dat$B2mg0[is.na(dat$B2mg_c)]
...
write.table(ddd,"10_5_feature_distribution_in_grade_0_3a_fill.txt",row.names = F,col.names = T,quote = F,sep = "\t")
ddd_r <-ddd
...
f3a<-f3a[,-6]
write.table(f3a,"10_5_feature_0_3a_chi_fill.txt",col.names = T,row.names = F,quote=F,sep = "\t")
