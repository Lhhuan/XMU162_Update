...
set.seed(123)
random_number <-sample(x=c(1:nrow(prog)), 0.2*nrow(prog),replace = F)
test_dat<-prog[random_number,]
train_dat <- setdiff(prog,test_dat)
write.table(train_dat,"03_train_3a_not_fill.txt",quote=F,sep="\t",row.names=FALSE)
write.table(test_dat,"03_test_3a_not_fill.txt",quote=F,sep="\t",row.names=FALSE)
# prog <-dat 
#----------------------------------------
...
write.table(prog,"01_add_age_raw_pfs_os_filter_grade_mergrPod_3A_fillna.txt",quote=F,sep="\t",row.names=FALSE)
test_dat<-prog[random_number,]
train_dat <- setdiff(prog,test_dat)
write.table(train_dat,"03_train_3a_fill.txt",quote=F,sep="\t",row.names=FALSE)
write.table(test_dat,"03_test_3a_fill.txt",quote=F,sep="\t",row.names=FALSE)
...
write.table(aa, "03_fill_univariate_cox_result_fill_3a.txt",quote=F,sep="\t",row.names=FALSE)
...
pdf("./figure/03_univariate_cox_fill_3a.pdf",width=5,height=5)
p1<-ggplot(data=aa, aes(x=reorder(var,X=HR),y=HR, ymin=HR_confint_lower , ymax=HR_confint_upper))
  ...
  p1 <-p1+geom_text(aes(x=var,y=10,label = significant)) +
...
res.cox <- coxph(Surv(os_month_new, dead) ~ CR+HGB+SUVmax+LDH+age_raw+B2mg+end_res+ECOG+interm_res+b2mg_LDH+X150b2mg_ldh+progress+LDH0, data =  prog)
x <- summary(res.cox)
...
aa$var <-gsub("_"," ",aa$var)
aa$var <-capitalize(aa$var)
pdf("./figure/03_Multivariable_cox_fill_3a.pdf",width=5,height=5)
p1<-ggplot(data=aa, aes(x=reorder(var,X=HR),y=HR, ymin=low , ymax=high))
  p1<-p1+geom_pointrange(size = 0.3) 
  p1<-p1+coord_flip()# +  # flip coordinates (puts labels on y axis)
  p1<-p1+labs(y="Hazard ratio",x="",title="Multivariable analysis") 
  p1 <-p1+p_theme+theme(axis.text.x = element_text(color = "black"),axis.text.y = element_text(color = "black"),plot.title = element_text(hjust = 0.5))
  p1 <-p1+geom_text(aes(x=var,y=17.5,label = significant)) +
      geom_hline(yintercept=1,colour="#c00024",linetype=3)
print(p1)
dev.off()


write.table(file="03_multivariate_cox_result_fill_3a.txt",multi_res,quote=F,sep="\t")
