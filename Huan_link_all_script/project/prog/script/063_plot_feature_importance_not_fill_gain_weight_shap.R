...
library(ggpubr)
library(Hmisc)
dat <- read.csv("062_ori_not_fill_feature_importance_weight.txt",na.strings = "",sep="\t")
...
pdf("./figure/063_not_fill_feature_importance_weight.pdf",width = 6,height = 6)
...
dev.off()
#--------------------gain
dat <- read.csv("062_ori_not_fill_feature_importance_gain.txt",na.strings = "",sep="\t")
dat$Feature <-as.factor(dat$Feature)
dat$Feature <-gsub("_"," ",dat$Feature)
dat$Feature <-gsub("Lym Mono","Lym/Mono",dat$Feature)
dat$Feature <-capitalize(dat$Feature) 
...
pdf("./figure/063_not_fill_feature_importance_gain.pdf",width = 6,height = 6)
p1 <-ggplot(data = dat, mapping = aes(x = reorder(Feature, Feature_importance), y = Feature_importance)) + geom_bar(stat = 'identity', fill = "#5eaaa8", width=0.5)+
...
#---------------------shap
dat <- read.csv("062_not_fill_feature_importance_shap_class.txt",na.strings = "",sep="\t")
dat <-abs(dat)
...
pdf("./figure/063_not_fill_feature_importance_shap.pdf",width = 6,height = 6)
