...
library(readr)
library(mygene)
setwd("~/project/link_database/geneontology/")



# tfs <- read_tsv(file_path('data/kinase.txt', col_names=F)
org<-read.table("01_unique_gene.csv.gz",header = T,sep = "\t") %>% as.data.frame()
g <-org$gene
...
# df.1$ensg <-unlist(a)
write.table(df.1,"02_symbol_ENSG.txt",quote = F,sep = "\t",row.names = F)
