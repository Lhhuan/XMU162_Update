...
class GCN(nn.Module):
    def __init__(self,
        ...
        self._in_src_feats, self._in_dst_feats = expand_as_pair(in_feats)   #私有变量
    ...
    def forward(self, graph, feat):
        with graph.local_scope():
            feat_src, feat_dst = expand_as_pair(feat, graph)  #feat_src源节点特征，feat_dst目标节点特征 #expand_as_pair用于指定图类型并将feature扩展为feat_src，feat_dst
            # (n, d_in, 1)
            graph.srcdata['h'] = feat_src.unsqueeze(-1)
            # (n, d_in, d_out)
            graph.update_all(fn.copy_u('h', 'm'), self.reducer('m', 'neigh')) #在图的一部分上进行消息传递 copy_u(u, out),u (str) – The source feature field. out (str) – The output message field.
            rst = graph.dstdata['neigh'].squeeze(-1)  # (n, d_out)node features.
            # residual connection
...
class Stretching_Potential_Loss(nn.Module):
    def __init__(self,r0):
        super(Stretching_Potential_Loss, self).__init__() #------拉伸潜在损失
        self.r0 = r0
...
class Electrostatic_Potential_Loss(nn.Module): #静电势损失
    def __init__(self):
...
class Loss(nn.Module):
    ...
    def forward(self, d,in_embed, out_embed,label):
        ...
        return loss.sum()
# model(train_graph,batched_center_word,batched_pos_word,batched_neg_word,0)
...
def load_data(setname):
    assert setname in ['01_train','01_val','01_test']
...
#-----------------------------------
def load_g():
    graph = load_graphs(os.path.join(root_path,r'undirectied_Graph.dgl'))[0][0]
    return graph
# graph = load_graphs(os.path.join(self.root_path,r'undirectied_Graph.dgl'))[0][0]
#----------------
root_path = '/public/huanhuan/project/output/'
# root_path= 'D:\deep_learning\GTEx\output'
batch_size = 1024
graph = load_graphs(os.path.join(root_path,r'01_undirectied_Graph.dgl'))[0][0]
...
#-----------------------------------
    def train(self):
        ...
        train_size,train_dataloader = load_data('01_train')
        val_size,val_dataloader = load_data('01_val')

        model = EmbeddingModel(nodes_num,256,8,1).to(gpu) # 实例化一个对象

        train_graph = load_graphs(os.path.join(root_path,r'01_train_graph.dgl'))[0][0].to(gpu)
        # val_graph = load_graphs(os.path.join(root_path,'data',r'val_graph.dgl'))[0][0].to(gpu)
        ...
        for epoch in range(1,epoches+1):
            ...
            for batched_center_word,batched_pos_word,batched_neg_word in train_bar:
                # graphdist = th.tensor(get_graphdist(batched_center_word,batched_neg_word)).cuda()
            ...
            if epoch == 1:
                pd.DataFrame(columns=['epoch','train_epoch_loss','test_epoch_loss']).to_csv(
                    os.path.join(root_path,r'record', rf'03_skipgram_gcn_date{date}_merics.txt'), sep='\t', index=None)
            pd.DataFrame([epoch,train_epoch_loss,test_epoch_loss]).T.to_csv(
                os.path.join(root_path,r'record', rf'03_skipgram_gcn_date{date}_merics.txt'), sep='\t', index=None,mode='a',header=None)
            if best_loss > test_epoch_loss:
                print(f"Now best model is epoch{epoch}")
                best_loss = test_epoch_loss
                lookup_table = model.lookup_table()
                np.save(os.path.join(root_path,r'model', rf'03_skipgram_gcn_date{date}_lookup_table.npy'),lookup_table)
                state = {'net': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
                th.save(state,os.path.join(root_path,r'model', rf'03_skipgram_gcn_date{date}_model.pkl'))

#-------------------------------------
root_path = '/home/huanhuan/project/GTEx/script/Tissue_merge/prediction/output/'
#-----------
source_file = pd.read_csv(r"/home/huanhuan/project/GTEx/script/Tissue_merge/prediction/output/01_merge_all_tissue_cis_sig_eQTL_hotspot_egene_idx.txt",sep='\t')
# lookup_table =  np.load("/public/home/huanhuan/project/GTEx/output/model/03_skipgram_gcn_date09_01_22_39_lookup_table.npy")
lookup_table =  np.load("/home/huanhuan/project/GTEx/script/Tissue_merge/prediction/output/model/03_skipgram_gcn_date11_15_21_59_lookup_table.npy")
...
def compute_acc(lookup_table,source_file):
    # split_point = source_file['diseaseidx'].max() + 1
    split_point = source_file['hotspotidx'].max() + 1
    ...
    undirected_graph = load_graphs("/home/huanhuan/project/GTEx/script/Tissue_merge/prediction/output/01_undirectied_Graph.dgl")[0][0]
    ...
    test_graph = load_graphs(r'/home/huanhuan/project/GTEx/script/Tissue_merge/prediction/output/01_test_graph.dgl')[0][0]
    ...
    #----huan add
    del(test_truth_table)
    del(undirected_truth_table)
    ...
    d=fpr,tpr,thresholds,optimal_th, optimal_point
    np.save(os.path.join(root_path,r'record', rf'03_skipgram_gcn_11_15_21_59_auc.npy'),d)

np.save(os.path.join(root_path,r'record', rf'03_skipgram_gcn_11_15_21_59_similarity_mtx.npy'),similarity_mtx)
roc_auc = auc(fpr,tpr)
plt.figure()
lw=2
plt.plot(fpr, tpr, color='darkorange',lw=lw, label='ROC curve (area = %0.2f)' % roc_auc)
# plt.plot(optimal_point,thresholds)
...
plt.savefig("/home/huanhuan/project/GTEx/script/Tissue_merge/prediction/output/figure/03_gcn_hotspot_egene_11_15_21_59__auc.png",dpi=600)#保存图片，dpi设置分辨率
plt.show()

#------------
# predict_link = similarity_mtx[undirected_truth_table > optimal_th]


similarity_df = pd.DataFrame(similarity_mtx)
similarity_df.to_csv("/home/huanhuan/project/GTEx/script/Tissue_merge/prediction/output/03_smiliarity.txt",sep="\t",index=None)




similarity_mtx.tofile("1.txt",sep='\t', format='%s')
