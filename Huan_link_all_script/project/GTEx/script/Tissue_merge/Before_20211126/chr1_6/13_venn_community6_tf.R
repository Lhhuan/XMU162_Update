...
library(ggpval)
library(VennDiagram)
setwd("/home/huanhuan/project/GTEx/script/Tissue_merge/chr1_6/6kmer/6_community")


tf6 <-read.table("knowResult_merge.txt",header = T,sep = "\t") %>% as.data.frame()
load("/home/huanhuan/project/GTEx/script/Tissue_merge/chr1_6/6kmer/5_community/bind_tf5.Rdata")
tf5<-filter(tf,community<5)
tf6<-filter(tf6,community>=5)
tf<-bind_rows(tf5,tf6)
...
AAA <- list(Community1=ff(1),Community2=ff(2),Community3=ff(3),Community4=ff(4),Community6=ff(6))
