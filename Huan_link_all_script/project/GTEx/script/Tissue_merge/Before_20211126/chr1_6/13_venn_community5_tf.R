...
library(ggpval)
library(VennDiagram)
setwd("/home/huanhuan/project/GTEx/script/Tissue_merge/chr1_6/6kmer/5_community")


tf5 <-read.table("knowResult_merge.txt",header = T,sep = "\t") %>% as.data.frame()
load("/home/huanhuan/project/GTEx/script/Tissue_merge/chr1_6/6kmer/4_community/bind_tf4.Rdata")
tf4 <-filter(tf,community<4)
tf5<-filter(tf5,community>=4)
tf<-bind_rows(tf4,tf5)
save(tf,file="bind_tf5.Rdata")
tf$key =paste(tf$Motif_Name,tf$Consensus,sep="_")
# ff <-function(i){
#     i_c  <- filter(tf,community==i)%>%select(Consensus)%>%as.matrix()%>%as.vector()    #%>%as.character()
#     return(i_c)
# }
# AAA <- list(Community1=ff(1),Community2=ff(2),Community3=ff(3),Community4=ff(4),Community5=ff(5))
