...
pca = prcomp_irlba(dat1, n = 9)
pca_result = pca$x
rownames(pca_result) = rownames(dat1)
...
ggsave("05_kmeans_tsne.png",p,dpi=300,width=10,height=10)
#_-------------------------------------------------------------------------
...
plist <- lapply(c(1:9),plot)
p =CombinePlots(plist,ncol=3,nrow=3)
ggsave("05_kmeans_UMAP.png",p,dpi=300,width=10,height=10)
