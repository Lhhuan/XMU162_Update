...
org<-read.table("/home/huanhuan/project/GTEx/script/Tissue_merge/figure/length_more10000_emplamda.bed",header = F,sep = "\t") %>% as.data.frame()
colnames(org) <-c("h_chr","h_start","h_end","h_len","q_chr","q_start","q_end","emplamda")

a <- org %>% group_by(h_chr,h_start,h_end,h_len)%>%summarise(count=n())%>%data.frame()
a$ratio = a$count/a$h_len
