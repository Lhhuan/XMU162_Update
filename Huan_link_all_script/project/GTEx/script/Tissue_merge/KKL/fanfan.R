...
pca_result = pca$x
# rownames(pca_result) = rownames(data)
# tsne = Rtsne::Rtsne(pca_result, pca = FALSE)
tsne = Rtsne::Rtsne(pca_result, pca = FALSE,check_duplicates = FALSE)
df = data.frame(tsne1 = tsne$Y[,1], tsne2 = tsne$Y[,2], cluster = ??)
ggplot(df) + 
geom_point(aes(x = tsne1, y = tsne2, colour = cluster), size = 0.8, alpha = 0.3)