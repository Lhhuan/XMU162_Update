...
set.seed(11)
Sorg <-Sorg[sample(1:nrow(Sorg_all), 5000, replace = FALSE),]
setwd("/home/huanhuan/project/eQTL_Catalogue/script/kmer/figure/random/")
...
ggsave("09_4096_PCA_screeplot.pdf",p1,dpi=300,width=10,height=5)
...
ggsave("09_4096_PCA_cluster_new.png",p,dpi=150,width=5,height=5)

tsne = Rtsne::Rtsne(pca_result[,1:30], pca = FALSE,check_duplicates = FALSE)
save(tsne,file="09_4096_new_pca_tsne.Rdata")
...
names(pca_list)=c("rbfdot","laplacedot","tanhdot","besseldot","anovadot","vanilladot","polydot")
save(pca_list,file = "09_4096_kpca.Rdata")
...
plist <-lapply(c(1:7),plot)
pdf("./09_4096_kpca_sampling_result.pdf",width=8.3, height=4)
...
pdf("./09_4096_kpca_umap_sampling_result.pdf",width=8.3, height=4)
CombinePlots(plist,ncol=4,nrow=2)
dev.off()
#--------------------------------------------tsne
tsne = Rtsne::Rtsne(pca_result[,1:30], pca = FALSE,check_duplicates = FALSE)
...
pdf("./09_4096_kpca_tsne_sampling_result.pdf",width=8.3, height=4)
CombinePlots(plist,ncol=4,nrow=2)
dev.off()
...
ggsave("09_4096_mclust_tsne_new.png",p,dpi=120,width=12,height=6)
...
p =CombinePlots(plist,ncol=4,nrow=2)
ggsave("09_4096_mclust_UMAP_new.png",p,dpi=120,width=12,height=6)
...
save(re,file="09_4096_kmeans_3_10.Rdata")
plot <-function(i){
    print(paste(i,"start",sep="\t"))
...
plist <- lapply(c(1:8),plot)
p =CombinePlots(plist,ncol=4,nrow=2)
ggsave("09_4096_kmeans_tsne.png",p,dpi=150,width=12,height=6)
#_-------------------------------------------------------------------------
...
plist <- lapply(c(1:8),plot)
p =CombinePlots(plist,ncol=4,nrow=2)
ggsave("09_4096_kmeans_UMAP.png",p,dpi=150,width=12,height=6)
...
cutoff <-quantile(eu_d,0.75) #28.80903
names(cutoff)=NULL

set.seed(111)
test <- sample(1:nrow(dat),size=100000,replace =T)
dat_test <-dat[test,]
...
p =ggplot(df) + geom_point(aes(x = UMAP_1, y = UMAP_2, colour = factor(cluster)), size = 0.3, alpha = 0.3)+theme_bw() + p_theme+
    guides(col=guide_legend(title='Cluster'))
ggsave("09_4096_leiden_Umap_cluster_new.png",p,dpi=300,width=5,height=5)
#----------------------------------------------------------------------------------cluster_louvain
g <-cluster_louvain(g1, weights = NULL)
df <-data.frame(UMAP_1=rumap[,1],UMAP_2=rumap[,2],cluster=g$membership)
p =ggplot(df) + geom_point(aes(x = UMAP_1, y = UMAP_2, colour = factor(cluster)), size = 0.3, alpha = 0.3)+theme_bw() + p_theme+
    guides(col=guide_legend(title='Cluster'))
ggsave("09_4096_louvain_Umap_cluster_new.png",p,dpi=300,width=5,height=5)
