...
org <-read.table("17_whole_genome_leiden_pca3_k50_resolution_2e-5_gwas_metaid.bed.gz",header = T,sep = "\t") 
...
pdf("18_boxpolt_whole_genome_gwas_trait.pdf",height=3.1,width=3)
print(p1)
dev.off()
...
pdf("18_boxpolt_whole_genome_gwas_trait_1E-8.pdf",height=3.1,width=3)
print(p1)
dev.off()
...
pdf("18_barpolt_whole_genome_GWAS.pdf",height=3.1,width=3)
print(p1)
dev.off()
...
dat4$cluster <-factor(dat4$cluster,levels=c(1,2))

p1 <- ggplot(dat4, aes(x=as.factor(cluster), y=log(trait_n+0.1),group=cluster,fill = as.factor(cluster))) + 
# geom_boxplot(outlier.colour = NA)+
geom_boxplot()+
# scale_fill_manual(values=c("#1E77B4","#FF7F0E","#2CA02C","#C22324","#9567BD","#8C554B","#E277C1","#7F7F7F","#BCBC22","#15BECE"))+
# scale_fill_manual(values=c("#8C554B","#C22324","#1E77B4","#FF7F0E","#9567BD","#2CA02C"))+
...
ggtitle("Number of trait per snp")+
theme(legend.position ="none")+
stat_compare_means(method = "wilcox.test",comparisons =list(c(1,2)),label="p.signif",method.args = list(alternative = "greater"))+
p_theme

pdf("18_boxpolt_whole_genome_trait_per_snp.pdf",height=3.1,width=3)
print(p1)
dev.off()
...
#===========
dat5$cluster <-factor(dat5$cluster,levels=c(1,2))
p1 <- ggplot(dat5, aes(x=as.factor(cluster), y=log(adjust_trait_number+0.1),group=cluster,fill = as.factor(cluster))) + 
# geom_boxplot(outlier.colour = NA)+
geom_boxplot()+
# scale_fill_manual(values=c("#1E77B4","#FF7F0E","#2CA02C","#C22324","#9567BD","#8C554B","#E277C1","#7F7F7F","#BCBC22","#15BECE"))+
# scale_fill_manual(values=c("#8C554B","#C22324","#1E77B4","#FF7F0E","#9567BD","#2CA02C"))+
...
pdf("18_boxpolt_whole_genome_trait_per_kb.pdf",height=3.1,width=3)
print(p1)
dev.off()