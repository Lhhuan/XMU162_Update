...
for (sample in sample_name){
    org <-read.table(paste0("/share/data0/QTLbase/huan/hic/",sample,"_5000.ginteractions_inter_chr_adjust.bed.gz"),header = F,sep = "\t") %>% as.data.frame()
    ...
    output_name<-paste0("/share/data0/QTLbase/huan/hic/",sample,"_5000.ginteractions_inter_chr_adjust_sum.bed")
    write.table(segment_score,output_name,row.names = F, col.names = F,quote =F,sep="\t")
    rm(org)
    sort_output_name <- paste0("/share/data0/QTLbase/huan/hic/",sample,"_5000.ginteractions_inter_chr_adjust_sum_sort.bed.gz")
    hotspot_hic <-paste0("hotspot_cluster_",sample,"_5000_inter_chr_adjust.bed.gz")
    system(paste0("less ",output_name,"|sort -k1,1 -k2,2n |gzip >",sort_output_name))
    system(paste0("gzip ",output_name))
    system(paste0("bedtools intersect -a ",sort_output_name," -b ../GWAS/17_whole_genome_leiden_pca3_k50_resolution1e-04_sorted.bed.gz -wo |gzip >",hotspot_hic))
    print(sample)
}
    ...
    fdat$mean_by_hotspot_length <- fdat$sum/fdat$length
    fdat$cluster <-factor(fdat$cluster,levels=c(4,2,1,5,6,3))
    ...
    p1 <- ggplot(fdat, aes(x=cluster, y=log(mean_by_hotspot_length),fill =cluster)) + 
        ...
        theme_bw()+
        ggtitle(paste0(title," (inter chr)"))+
        p_theme +
        labs(x="Cluster",y=paste0("Log(mean(signal of ",type,"))"))
    pdf(paste0("25_boxplot_",title2,"_signal_mean_by_hotspot_length_inter_chr.pdf"),height=4.7,width=4.5)
    print(p1)
    dev.off()
    ...
    pdf(paste0("25_boxplot_",title2,"_mean_overlap_by_hotspot_length_inter_chr.pdf"),height=4.7,width=4.5)
    print(p1)
    dev.off()
    print(sample)
}