...
all_gene <-read.table("/home/huanhuan/project/eQTL_Catalogue/output/all_tissue_status/hotspot/Tissue_merge_segment_hotspot_cutoff_0.176_extend_18_snp_sorted_merge_egene_eQTL_5e_8_all_info.bed.gz",header = F,sep = "\t") %>% as.data.frame()
...
colnames(all_gene)[c(7,9)]<-c("ENSEMBL","Tissue")
all_gene<-left_join(all_gene,Cluster[,c("cluster","hotspot")],by="hotspot")
all_gene<-all_gene[,c("cluster","hotspot","eqtl","ENSEMBL","Tissue")]%>%unique()

all_gene_count <-all_gene%>%group_by(cluster,Tissue)%>%summarise(eqtl_gene_count=n())%>%data.frame()
tissue_eqtl <- all_gene%>%group_by(Tissue)%>%summarise(tissue_eqtl_count=n())%>%data.frame()
all_gene_count <-left_join(all_gene_count,tissue_eqtl,by="Tissue")
all_gene_count$tissue_ratio <- all_gene_count$eqtl_gene_count/all_gene_count$tissue_eqtl_count
eqtl_n <- all_gene%>%group_by(cluster)%>%summarise(eqtl_count=n())%>%data.frame()
all_gene_count <-left_join(all_gene_count,eqtl_n,by="cluster")
all_gene_count$eqtl_ratio <- all_gene_count$eqtl_gene_count/all_gene_count$eqtl_count
write.table(all_gene_count,"13_2_cluster_tissue_specific.txt",row.names = F, col.names = T,quote =F,sep="\t")
...
write.table(cluster_chr,"13_2_cluster_chr_specific.txt",row.names = F, col.names = T,quote =F,sep="\t")
