...
# df = data.frame(dat[,1:4], umap, tsne$Y, as.factor(c$membership) )
# colnames(df) = c(paste0("PC_",1:4), paste0("UMAP_",1:2), paste0("tSNE_",1:2), "cluster")
...
df = data.frame(dat[,1:5], umap, tsne$Y, as.factor(g$membership) )
# df = data.frame(dat[,1:4], umap, tsne$Y, as.factor(g$membership) )
colnames(df) = c(paste0("PC_",1:5), paste0("UMAP_",1:2), paste0("tSNE_",1:2), "cluster")
df$hotspot <- rownames(as.data.frame(pca1@xmatrix))[as.numeric(rownames(dat))]
...
dup <-data.frame(dat_dup[,1:5],hotspot=rownames(as.data.frame(pca1@xmatrix))[as.numeric(rownames(dat_dup))])
colnames(dup)[1:5] <-paste0("PC_",1:5)
dup <-left_join(dup,df[,1:10],by=c("PC_1","PC_2","PC_3","PC_4","PC_5"))
dup <-dup[,c(1:5,7:11,6)]
