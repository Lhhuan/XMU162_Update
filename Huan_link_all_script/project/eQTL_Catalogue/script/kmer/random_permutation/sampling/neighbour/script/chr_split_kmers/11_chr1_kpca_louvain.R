...
set.seed(123)
umap <- uwot::umap(dat)
save(umap,file="10_chr1_5kpca_umap.Rdata")
set.seed(123)
tsne = Rtsne::Rtsne(dat, initial_dims = pcn, pca = FALSE)
save(tsne,file="10_chr1_5kpca_tsne.Rdata")
# clustering
k = 500
...
p <- ggplot(df, aes(x = PC_1, y = PC_2, color = cluster)) + 
  ...
  theme_bw()
ggsave(paste0("11_chr1_louvain_pca",pcn,"_k",k,"_pca.png"), p, width = 6, height = 5.3)

p <- ggplot(df, aes(x = tSNE_1, y = tSNE_2, color = cluster)) + 
  geom_point(size = 2, alpha = 0.6) + 
  scale_color_d3("category20") + 
  theme_bw()
ggsave(paste0("11_chr1_louvain_pca",pcn,"_k",k,"_tsne.png"), p, width = 6, height = 5.3)

p <- ggplot(df, aes(x = UMAP_1, y = UMAP_2, color = cluster)) + 
  geom_point(size = 2, alpha = 0.6) + 
  scale_color_d3("category20") + 
  theme_bw()
ggsave(paste0("11_chr1_louvain_pca",pcn,"_k",k,"_umap.png"), p, width = 6, height = 5.3)

write.table(df,paste0("11_chr1_louvain_pca",pcn,"_k",k,".txt"),row.names = F, col.names = T,quote =F,sep="\t")
