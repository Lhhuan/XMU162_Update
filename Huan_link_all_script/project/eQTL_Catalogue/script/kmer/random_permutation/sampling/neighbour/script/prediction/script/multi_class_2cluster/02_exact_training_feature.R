...
posi_gc <- read.csv("/home/huanhuan/project/eQTL_Catalogue/script/kmer/random_permutation/sampling/neighbour/output/figure/whole_genome/3pca_2cluster/GC_content/gc_content.bed.gz",header = T,sep = "\t") %>% as.data.frame()
colnames(posi_gc)[1:4] <-c("Chr","start","end","GC_content")
posi_gc$hotspot <-paste0(posi_gc$Chr,":",posi_gc$start,"-",posi_gc$end)
posi_gc$Class <-posi_gc$cluster
...
write.table(ALL_train,"02_whole_genome_training_dataset_nega_large.txt",row.names = F, col.names = T,quote =F,sep="\t")
system("gzip 02_whole_genome_training_dataset_nega_large.txt")


set.seed(123)
nega1 <- nega[sample(nrow(nega),size=20000),]
ALL_train1 <- rbind(nega1,posi)
write.table(ALL_train1,"02_whole_genome_training_dataset_nega2w.txt",row.names = F, col.names = T,quote =F,sep="\t")
system("gzip 02_whole_genome_training_dataset_nega2w.txt")



Tchr1 <- filter(ALL_train,Chr=="chr1")
write.table(Tchr1,"02_chr1_training_dataset_negaL.txt",row.names = F, col.names = T,quote =F,sep="\t")
system("gzip 02_chr1_training_dataset_negaL.txt")
