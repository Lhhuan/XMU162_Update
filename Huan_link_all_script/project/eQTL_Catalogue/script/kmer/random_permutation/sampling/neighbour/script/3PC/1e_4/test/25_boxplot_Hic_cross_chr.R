...
org <-read.table("hotspot_cluster4DNFIQYQWPF5_5000_cross_chr.bed.gz",header = F,sep = "\t") %>% as.data.frame()
...
p1 <- ggplot(all_hotspot, aes(x=cluster, y=log(mean_by_hotspot_length),fill =cluster)) + 
    ...
    theme_bw()+
    ggtitle("in situ Hi-C on H1-hESC (inter chr)")+
    p_theme +
    # labs(x="Cluster",y="mean(signal of hic)")
    labs(x="Cluster",y="Log(mean(signal of hic))")
pdf("25_boxplot_H1-hESC_signal_mean_by_hotspot_length_inter_chr.pdf",height=4.7,width=4.5)
print(p1)
dev.off()