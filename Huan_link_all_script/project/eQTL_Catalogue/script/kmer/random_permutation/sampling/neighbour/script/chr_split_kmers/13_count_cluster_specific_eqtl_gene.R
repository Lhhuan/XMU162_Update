...
chr1_gene$eqtl <- paste0(chr1_gene$V4,":",chr1_gene$V5,"-",chr1_gene$V6)
chr1_gene<-left_join(chr1_gene,Cluster[,c("cluster","hotspot")],by="hotspot")
colnames(chr1_gene)[7]<-"ENSEMBL"
chr1_gene<-chr1_gene[,c("cluster","hotspot","eqtl","ENSEMBL")]
chr1_gene_count <-chr1_gene%>%group_by(hotspot,cluster)%>%summarise(eqtl_gene_count=n())%>%data.frame()


p1 <- ggplot(chr1_gene_count, aes(x=as.factor(cluster), y=eqtl_gene_count,group=cluster,fill = as.factor(cluster))) + 
    geom_boxplot()+
...
p <- ggplot(chr1_gene_count, aes(x=eqtl_gene_count,color=as.factor(cluster))) + 
geom_density()+
...
pdf("13_densityplot_chr1_eqtl_gene_count.pdf",height=6,width=7)
print(p)
dev.off()
