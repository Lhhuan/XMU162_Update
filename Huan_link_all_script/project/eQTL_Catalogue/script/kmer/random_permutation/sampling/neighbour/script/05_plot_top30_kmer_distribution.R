...
dat <- read.table(paste0("03_hotspot_10_neighbour_random_kmer_wilcox_twoside.txt"),header = T,sep = "\t") %>% as.data.frame()
sdat<-filter(dat,p_value <0.05)
sdat <-sdat[order(sdat$p_value),]
tsg <-sdat[1:30,]
