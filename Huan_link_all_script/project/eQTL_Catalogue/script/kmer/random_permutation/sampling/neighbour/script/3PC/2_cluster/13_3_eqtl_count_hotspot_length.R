...
eqtl_n$cluster <-factor(eqtl_n$cluster,levels=c(1,2))
...
pdf("13_3_boxplot_of_adjust_Number_of_eQTL_per_kb.pdf",height=3.1,width=3)
print(p1)
dev.off() 
...
eQTL_egene_n$cluster <-factor(eQTL_egene_n$cluster,levels=c(1,2))
...
p1 <- ggplot(eQTL_egene_n, aes(x=cluster, y=log(ENSG_n),fill =cluster)) + 
    ...
    scale_fill_manual(values=c("#1E77B4","#FF7F0E"))+
    theme_bw()+
    ggtitle("Number of egene per eQTL")+
    stat_compare_means(method = "wilcox.test",comparisons =list(c(1,2)),label="p.signif",method.args = list(alternative = "less"))+
    p_theme +
    # labs(x="Cluster",y="mean(signal of hic)")
    labs(x="Cluster",y="log(Number of egene per eQTL)")
pdf("13_3_boxplot_of_Number_of_egene_per_eQTL.pdf",height=3.2,width=3)
print(p1)
dev.off() 
...
egene_n$cluster <-factor(egene_n$cluster,levels=c(1,2))
...
pdf("13_3_boxplot_of_Number_of_eQTL_per_egene.pdf",height=3.1,width=3)
print(p1)
dev.off() 
...
egene_n1$cluster <-factor(egene_n1$cluster,levels=c(1,2))
