...
library(ggpubr)
library(Hmisc)
dat <- read.csv("041_XGB2_model_feature_importance_weight.txt",na.strings = "",sep="\t")
...
pdf("06_top20_feature_importance_weight.pdf",width = 4,height = 4)
print(p1)
dev.off()
#
#--------------------gain
dat <- read.csv("041_XGB2_model_feature_importance_gain.txt",na.strings = "",sep="\t")
dat$Feature <-as.factor(dat$Feature)
dat$Feature <-gsub("_"," ",dat$Feature)
dat$Feature <-gsub("CHROMATIN Accessibility","CA",dat$Feature)
...
p1 <-ggplot(data = dat[1:20,], mapping = aes(x = reorder(Feature, Feature_importance), y = Feature_importance)) + geom_bar(stat = 'identity', fill = "#1A5599", width=0.8)+
  ...
  theme(axis.text.y = element_text(color="black"),
    axis.text.x = element_text(color="black"))
pdf("06_top20_feature_importance_gain.pdf",width = 4,height = 4)
print(p1)
dev.off()
#---------------------shap
