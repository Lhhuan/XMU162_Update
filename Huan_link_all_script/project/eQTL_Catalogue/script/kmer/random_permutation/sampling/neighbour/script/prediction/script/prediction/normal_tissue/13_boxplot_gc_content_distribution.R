...
for (i in 1:length(tissues)){
    tissue = tissues[i]
    new_dir <- paste0("/share/data0/QTLbase/huan/eQTL_Catalogue/prediction/normal_tissue/",tissue,"/GC_content")
    ...
    plist[[i]] <- ggplot(org1, aes(x=cluster, y=GC_content,fill =cluster)) + 
        ...
        # labs(y="GC content",x="")
    pdf("13_gc_content_boxplot.pdf",height=3.1,width=3)
    print(plist[[i]])
    dev.off()
    print(c(i,tissue))
}
...
pdf("13_ALL_tissue_boxplot_gc_content.pdf",width=12, height=9.5)
