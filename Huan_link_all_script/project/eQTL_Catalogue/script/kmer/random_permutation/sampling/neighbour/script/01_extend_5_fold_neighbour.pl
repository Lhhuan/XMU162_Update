...
my $fo1 = "../output/01_extend_5_fold_neighbour.bed.gz";
...
while(<$I2>)
{
    ...
    my $end=$f[2];
    my $length = $end - $start;
    my $new_start =$start - 5*$length;
    my $new_end= $end + 5*$length;
...
close($O1);
system "zless $fo1 |sort -k1,1 -k2,2n |gzip > ../output/01_extend_5_fold_neighbour_sort.bed.gz";
system "bedtools merge -i  ../output/01_extend_5_fold_neighbour_sort.bed.gz |gzip > ../output/01_extend_5_fold_neighbour_sort_merge.bed.gz";
