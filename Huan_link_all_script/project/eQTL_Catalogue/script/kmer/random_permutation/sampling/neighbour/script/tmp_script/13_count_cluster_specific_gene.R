...
setwd("/home/huanhuan/project/eQTL_Catalogue/script/kmer/random_permutation/sampling/neighbour/output/figure/09_chr/chr1/gene/")
...
p1 <- ggplot(chr1_gene_count, aes(x=as.factor(cluster), y=gene_count,group=cluster,fill = as.factor(cluster))) + 
    geom_boxplot()+
    labs(x="Cluster",y="Number of eGene")+
    theme_bw()+
    theme(legend.position="none")
pdf("13_gene_count_boxplot.pdf",height=6,width=6)
print(p1)
dev.off()

p <- ggplot(chr1_gene_count, aes(x=gene_count,color=as.factor(cluster))) + 
geom_density()+
scale_color_d3("category20") + 
theme_bw()+
xlab("Number of eGene") #+
# coord_cartesian(xlim = c(0, 50))

pdf("13_densityplot_chr1_gene_count.pdf",height=6,width=7)
print(p)
dev.off()
...
library(VennDiagram)
ff <-function(i){
    i_c  <- filter(chr1_gene,cluster==i)%>%select(ENSEMBL)%>%unique()%>%as.matrix()%>%as.vector()    #%>%as.character()
    return(i_c)
}
# AAA <- list(C1=ff(1),C3=ff(3))
...
# my_c2 =c( "darkorange1", "seagreen3")
my_c2 =c("dodgerblue", "goldenrod1", "darkorange1", "seagreen3","#beca5c")

pdf("13_Venn_Cluster1_3_egene.pdf",height=5,width=5)
...
pdf("13_Venn_Cluster1_5_egene.pdf",height=5,width=5)
 P1 <- venn.diagram(x=AAA,filename =NULL,lwd=1,lty=2,col=my_c2 ,fill=my_c2,reverse=TRUE,cex=0.8,fontface = "bold",cat.cex=0.9,cat.fontfamily = "serif")
#  print(P1)
grid.draw(P1)
dev.off()
