...
negative_signal<-function(marker= NULL){
    file_name <-paste0(marker,"_1_resemble_Tissue_merge_segment_hotspot_cutoff_0.176_extend_18_snp_sorted_merge.bed.gz")
    ...
    output_file <-paste0(marker,"_max_mean_median_1_resemble_Tissue_merge_segment_hotspot_cutoff_0.176_extend_18_snp_sorted_merge.bed")
    write.table(dat,output_file,row.names = F, col.names = T,quote =F,sep="\t")
    print(marker)
    system(paste("gzip",output_file,sep=" "))
}

lapply(markers,negative_signal)
