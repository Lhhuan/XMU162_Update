...
setwd("/home/huanhuan/project/eQTL_Catalogue/script/kmer/random_permutation/sampling/neighbour/output/figure/whole_genome/using_umap_clustering/3pca_1e_5/GWAS/")
org <-read.table("17_whole_genome_umap_leiden_pca3_k30_resolution1e-05_gwas_metaid.bed.gz",header = T,sep = "\t") %>% as.data.frame()
...
all_hotspot <- read.table("../../../17_whole_genome_umap_leiden_pca3_k30_resolution1e-05_sorted.bed.gz",header = F,sep = "\t") %>% as.data.frame()
...
dat1 <-filter(all_hotspot,cluster %in%c(1:10))
