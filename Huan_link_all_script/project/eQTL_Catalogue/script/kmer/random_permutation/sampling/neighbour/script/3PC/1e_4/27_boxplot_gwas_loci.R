...
org$loci <- paste(org$GWAS_chr,org$GWAS_start,org$GWAS_end,sep="_")
org <-org[,c("hotspot","cluster","length","loci")]%>%unique()
dat <-org%>%group_by(hotspot,length,cluster)%>%summarise(loci_count=n())%>%data.frame()
...
all_hotspot$loci_count[is.na(all_hotspot$loci_count)] <-0
all_hotspot$adjust_loci_count<-all_hotspot$loci_count/all_hotspot$length*1000
...
p1 <- ggplot(dat1, aes(x=as.factor(cluster), y=log(adjust_loci_count),group=cluster,fill = as.factor(cluster))) + 
...
pdf("27_boxpolt_whole_genome_gwas_loci_GWAS_catalogy.pdf",height=4.7,width=4.5)
print(p1)
dev.off()



p1 <- ggplot(dat1, aes(x=as.factor(cluster), y=log(adjust_loci_count+0.1),group=cluster,fill = as.factor(cluster))) + 
# p1 <- ggplot(dat1, aes(x=as.factor(cluster), y=adjust_loci_count,group=cluster,fill = as.factor(cluster))) + 
# geom_boxplot(outlier.colour = NA)+
geom_boxplot()+
# scale_fill_manual(values=c("#1E77B4","#FF7F0E","#2CA02C","#C22324","#9567BD","#8C554B","#E277C1","#7F7F7F","#BCBC22","#15BECE"))+
# scale_fill_manual(values=c("#8C554B","#C22324","#1E77B4","#FF7F0E","#9567BD","#2CA02C"))+
...
pdf("27_boxpolt_whole_genome_gwas_loci_GWAS_catalogy_1E-8.pdf",height=4.7,width=4.5)
print(p1)
dev.off()

f4 <-filter(dat1,cluster==4)
p <- ggplot(f4, aes(x=log(adjust_loci_count+0.001))) + 
...
pdf("27_densityplot_C4_gwascatalogy_loci.pdf",height=4.5,width=4.9)
print(p)
dev.off()
...
aa <-filter(dat1,adjust_loci_count>0)
...
pdf("27_barpolt_whole_genome_GWAS_catalogy_loci.pdf",height=4.7,width=4.5)
print(p1)
dev.off()