...
org <-read.table("hotspot_cluster_4DNFIFLJLIS5_5000_cross_chr.bed.gz",header = F,sep = "\t") %>% as.data.frame()
...
p1 <- ggplot(all_hotspot, aes(x=cluster, y=log(mean_by_hotspot_length),fill =cluster)) + 
    ...
    theme_bw()+
    ggtitle("in situ Hi-C on HFFc6 (inter chr)")+
    p_theme +
    # labs(x="Cluster",y="mean(signal of hic)")
    labs(x="Cluster",y="Log(mean(signal of hic))")
pdf("25_boxplot_HFFc6_signal_mean_by_hotspot_length_cross_chr.pdf",height=4.7,width=4.5)
print(p1)
dev.off()