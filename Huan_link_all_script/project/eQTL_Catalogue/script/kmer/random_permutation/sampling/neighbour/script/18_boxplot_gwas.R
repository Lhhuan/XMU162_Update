...
setwd("/home/huanhuan/project/eQTL_Catalogue/script/kmer/random_permutation/sampling/neighbour/output/figure/whole_genome/GWAS/")
...
all_hotspot <- read.table("../17_whole_genome_leiden_pca5_k30_resolution1e-04_sorted.bed.gz",header = F,sep = "\t") %>% as.data.frame()
...
p1 <- ggplot(all_hotspot, aes(x=as.factor(cluster), y=log(adjust_trait_count),group=cluster,fill = as.factor(cluster))) + 
# p1 <- ggplot(all_hotspot, aes(x=as.factor(cluster), y=adjust_trait_count,group=cluster,fill = as.factor(cluster))) + 
# geom_boxplot(outlier.colour = NA)+
geom_boxplot()+
scale_fill_manual(values=c("#1E77B4","#FF7F0E","#2CA02C","#C22324","#9567BD","#8C554B","#E277C1","#7F7F7F","#BCBC22","#15BECE"))+
theme_bw()+
labs(x="Cluster",y="Log(number of snp-trait pairs)")+
...
p <- ggplot(all_hotspot, aes(x=log(adjust_trait_count),color=cluster)) + 
geom_density()+
scale_color_d3("category20") + 
theme_bw()+
labs(x="Log(Number of snp-trait pairs)",color="Cluster") +
ggtitle("SNP-trait pairs")+
p_theme
