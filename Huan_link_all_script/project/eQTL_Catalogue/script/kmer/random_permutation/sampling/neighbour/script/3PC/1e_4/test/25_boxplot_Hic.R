...
org <-read.table("hotspot_cluster4DNFIQYQWPF5_5000.bed.gz",header = F,sep = "\t") %>% as.data.frame()
...
p1 <- ggplot(all_hotspot, aes(x=cluster, y=log(mean_by_hotspot_length+0.00001),fill =cluster)) + 
    geom_boxplot()+
    ...
    theme_bw()+
    ggtitle("in situ Hi-C on H1-hESC")+
    p_theme +
    # labs(x="Cluster",y="mean(signal of hic)")
    labs(x="Cluster",y="Log(mean(signal of hic))")
pdf("25_boxplot_H1-hESC_signal_mean_by_hotspot_length.pdf",height=4.7,width=4.5)
print(p1)
dev.off()