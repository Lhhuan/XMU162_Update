...
org <-read.table("17_chr1_louvain_pca10_k300_hotspot_gwas_metaid.bed.gz",header = T,sep = "\t") %>% as.data.frame()
...
geom_boxplot()+
theme_bw()
pdf("18_boxpolt_chr1_gwas_trait.pdf",height=6,width=7)
print(p1)
dev.off()



p <- ggplot(dat, aes(x=trait_count,color=as.factor(cluster))) + 
geom_density()+
