...
org<-read.csv("06_predicted_warm_regions_6mers_uc_us_no_log.csv.gz",header = T,sep = ",") %>% as.data.frame()
...
nega_gc <- read.csv("06_predicted_warm_regions.gc_content.bed.gz",header = T,sep = "\t") %>% as.data.frame()
