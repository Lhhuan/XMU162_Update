...
setwd("/home/huanhuan/project/eQTL_Catalogue/script/kmer/random_permutation/sampling/neighbour/output/figure/whole_genome/3pca_2e_4/gene/")
...
Cluster <-read.table("../../11_whole_genome_leiden_pca3_k50_resolution2e-04.txt",header = T,sep = "\t") %>% as.data.frame()
...
all_gene_count1 <-filter(all_gene_count,cluster %in%c(1:11))
...
p1 <- ggplot(all_gene_count1, aes(x=as.factor(cluster), y=log(adjust_eqtl_gene_count),group=cluster,fill = as.factor(cluster))) + 
    ...
    scale_fill_manual(values=c("#1E77B4","#FF7F0E","#2CA02C","#C22324","#9567BD","#8C554B","#E277C1","#7F7F7F","#BCBC22","#15BECE","#A6BDDD"))+
