...
permutation_sig <- fdat0
# permutation_sig <-permutation_sig[order(permutation_sig$p_value),]
# tsg <-permutation_sig[1:30,]
load("./figure/05_wilcox_plot_kmer.Rdata") #tsg
load("06_random_kmer_need_test_ratio_1_1000.Rdata")
...
aaa <-filter(tsg,seq %in%permutation_sig$seq)
unique_hotspot_kmer_need_test1 <-as.character(unique(aaa$seq))
