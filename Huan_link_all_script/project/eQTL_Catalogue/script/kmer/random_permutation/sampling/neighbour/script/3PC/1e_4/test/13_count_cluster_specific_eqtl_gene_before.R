...
all_gene_count1$cluster <-factor(all_gene_count1$cluster,levels=c(6,4,1,2,5,3))
...
for(type in  types){
    ...
    for(j in c(1:length(aaa))){
        ...
        write.table(specific,paste0("./",CC,"/",CC,"_",type,"_specific_.txt"),col.names=F,row.names =F,quote=F,sep="\t")
    }
}
