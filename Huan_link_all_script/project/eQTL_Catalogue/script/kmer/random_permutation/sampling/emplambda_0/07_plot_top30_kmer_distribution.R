...
load("./output/all_random_kmer_need_test_ratio_1_100.Rdata")
load("./output/06_permutation_test_100_sig_kmer.Rdata")
hotf<-read.table("/home/huanhuan/project/eQTL_Catalogue/output/all_tissue_status/hotspot/Tissue_merge_segment_hotspot_cutoff_0.176_extend_18_snp_sorted_merge.bed.gz",header = T,sep = "\t")%>% as.data.frame()

or1 <-fdat0[order(fdat0$p_value),]
tsg <-or1[1:30,]
...
pdf("./07_sig_kmer_distrbution.pdf",width=12, height=10)
CombinePlots(plist,ncol=6,nrow=5)
dev.off()
print(111)

pdf("./07_sig_kmer_distrbution_10.pdf",width=8, height=4.1)
