...
org<-read.table("/home/huanhuan/project/eQTL_Catalogue/output/all_tissue_status/hotspot/Tissue_merge_segment_hotspot_cutoff_0.176_extend_sorted_merge.bed.gz",header = F,sep = "\t") %>% as.data.frame()
...
#----------------------------
org$class <-NA
org$class[which(org$hotspot_length <=100 )] <- "<=100"
org$class[which(org$hotspot_length <=1000 & org$hotspot_length >100)] <- "101-1,000"
org$class[which(org$hotspot_length <=5000 & org$hotspot_length >1000)] <- "1,001-5,000"
org$class[which(org$hotspot_length <=10000 & org$hotspot_length >5000)] <- "5,001-10,000"
org$class[which(org$hotspot_length <=100000 & org$hotspot_length >10000)] <- "10,001-100,000"
org$class[which(org$hotspot_length >100000)] <- ">100,000"

org$class<-factor(org$class,levels=c("<=100","101-1,000","1,001-5,000","5,001-10,000","10,001-100,000",">100,000"))
l_c <- table(org$class)%>%data.frame()
colnames(l_c) <-c("Length","Frequency")

write.table(l_c,"041_hotspot_distribution_bar.txt",row.names=F,quote=F,sep="\t")

pdf("./041_barplot_hotspot_distribution.pdf",width = 4,height = 4)
