...
my $f2 = "../output/all_tissue_status/hotspot/Tissue_merge_segment_hotspot_cutoff_0.176.bed.gz";
# open my $I1, '<', $f1 or die "$0 : failed to open input file '$f1' : $!\n"; 
open( my $I2 ,"gzip -dc $f2|") or die ("can not open input file '$f2' \n"); #读压缩文件
my $fo3 = "../output/all_tissue_status/hotspot/Tissue_merge_segment_hotspot_cutoff_0.176_extend.bed.gz";
...
while(<$I2>)
{
    ...
    # my $true_end= $end -1;
    my $true_end= $end;
    my $true_start = $start+1;
    my $start_k= "$chr\t$true_start";
...
close($O3);
system "zless $fo3 |sort -k1,1 -k2,2n |gzip > ../output/all_tissue_status/hotspot/Tissue_merge_segment_hotspot_cutoff_0.176_extend_sorted.bed.gz"