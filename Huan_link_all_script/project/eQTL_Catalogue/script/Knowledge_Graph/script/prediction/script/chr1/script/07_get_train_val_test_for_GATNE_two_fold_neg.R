...
test_n <- potential_negative[1:(nrow(test_p)*2),]
val_n  <- potential_negative[(nrow(test_p)*2+1):(nrow(test_p)*2+nrow(val_p)*2),]
...
write.table(test,"./huan_GATNE_neg_2fold/test.txt",row.names = F, col.names = F,quote =F,sep=" ")
write.table(val,"./huan_GATNE_neg_2fold/valid.txt",row.names = F, col.names = F,quote =F,sep=" ")
#---------
...
train <-train[,c(3,1,2)]
write.table(train,"./huan_GATNE_neg_2fold/train.txt",row.names = F, col.names = F,quote =F,sep=" ")

train_hf <-train_h[,c(3,1,2)]
write.table(train_hf,"./huan_GATNE_neg_2fold/train.txt",row.names = F, col.names = F,quote =F,sep=" ")
