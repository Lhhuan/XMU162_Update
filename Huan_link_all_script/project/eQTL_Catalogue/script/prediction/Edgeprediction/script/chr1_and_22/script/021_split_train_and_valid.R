...
org<-read.table("02_chr1_22_hotspot.csv.gz",header = T,sep = ",") %>% as.data.frame()

set.seed(123)
valid <- org[sample(nrow(org),round(nrow(org)*0.1)),]
# train <-setdiff(org,valid)
write.table(valid,"./test/021_chr1_and_22_hotsopt_valid.csv",quote = F,sep = ",",row.names = F)
# write.table(train,"./train/021_hotsopt_train.csv",quote = F,sep = ",",row.names = F)
# gzip("./train/021_hotsopt_train.csv")
gzip("./test/021_chr1_and_22_hotsopt_valid.csv")