###"../output/all_tissue_status/NHP/NHPoisson_emplambda_interval_${j}_cutoff_7.3_${tissue}.txt.gz" 得hotspot(segment),"../output/all_tissue_status/hotspot/${tissue}_segment_hotspot_cutoff_${cutoff}.bed.gz"
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
my $f1 = "../output/all_tissue_status/NHP/NHPoisson_emplambda_interval_${j}_cutoff_7.3_${tissue}.txt.gz";
...
my $output_dir = "../output/all_tissue_status/hotspot/";
unless(-e $output_dir){
        ...
        if ($emplambda >= $cutoff){
            ...
            if ($content_count >0){
                ...
                if ($end_diff <2 ){ #距离hotspot 一行的位置
                    ...
                    my $bed_ends = $ends[0]+1-1;#避免文件出现科学计数法(e+)
                    my $bed_starts= $starts[0]-1; #避免文件出现科学计数法(e+)
