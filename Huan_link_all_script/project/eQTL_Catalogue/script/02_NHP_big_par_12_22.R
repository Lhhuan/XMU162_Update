...
get_nhp <- function(i = NULL){
    # ProcessBedGz <- function(i = NULL){
    print(c(i,"start"))
    org2 <- read.table(paste0("../../chr_split/01_merge_all_tissue_cis_eQTL_1kg_Completion_chr",i,".txt.gz"),header = T,sep = "\t") %>% as.data.frame()
    ...
    # dev.off()
    rs2<-cbind(emplambda=emplambdaB$emplambda, t=emplambdaB$t,chr=i) %>% as.data.frame()
    print(i)
    output_file2<-paste("CHR_",i,"_NHPoisson_emplambda_interval_",j,"_cutoff_7.3_",name,".txt", sep = "")
...
#----------------------------------------------
setwd("/home/huanhuan/project/eQTL_Catalogue/output/all_tissue_status/NHP")
name= "Tissue_merge"
j=18
cutoff =-log10(5e-8)
# lapply(2:10,get_nhp)
mclapply(12:22, get_nhp, mc.cores = 2)
...
# setwd("/home/huanhuan/project/eQTL_Catalogue/output/all_tissue_status/NHP")
# org <- read.table("../../01_merge_all_tissue_cis_eQTL_1kg_Completion.txt.gz",header = F,sep = "\t") %>% as.data.frame()
