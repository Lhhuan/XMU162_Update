...
# Unregister browser native host manifest before killing Connect (to avoid restart)
if [ "$system_wide_install" = "no" ]; then
  "$HOME/Applications/Aspera Connect.app/Contents/MacOS/asperaconnect-nmh" --unregister
else
  "/Applications/Aspera Connect.app/Contents/MacOS/asperaconnect-nmh" --unregister
fi
...
ps -Ax | grep -i "IBM Aspera Launcher" | sed /grep/d | awk '{ print $1 }' | xargs kill -9
...
if [ "$is_admin" == "yes" ]; then
  ...
  remove_directory /Applications/IBM\ Aspera\ Launcher.app
...
if [ "$user_install" = "yes" -o "$force" = "yes" ]; then
  ...
  remove_directory "$user_install_root/Applications/IBM Aspera Launcher.app"
  ...
  if [ -e "$user_install_root/Library/Logs/IBM_Aspera_Connect" ]; then
    if [ "$(ls -A "$user_install_root/Library/Logs/IBM_Aspera_Connect")" ]; then
      echo "Keeping $user_install_root/Library/Logs/IBM_Aspera_Connect directory since it is not empty"
    else
      remove_directory "$user_install_root/Library/Logs/IBM_Aspera_Connect"
    fi
  fi
  if [ -e "$user_install_root/Library/Logs/Aspera_Connect" ]; then
    if [ "$(ls -A "$user_install_root/Library/Logs/Aspera_Connect")" ]; then
      echo "Keeping $user_install_root/Library/Logs/Aspera_Connect directory since it is not empty"
    else
      remove_directory "$user_install_root/Library/Logs/Aspera_Connect"
    fi
  fi
...
if [ $remove_preferences = "yes" ]; then
  ...
  if [ "$user_install_root" = "/Library/" ]; then
      # Remove leftover files in users' personal folders if the user didn't install connect as a per-user application
      for user in $(ls /Users); do
          if [ -d "/Users/$user" ]; then
              if [ ! -e "/Users/$user/Applications/Aspera Connect.app" ]; then
                  remove_directory "/Users/$user/.aspera/connect"
                  if [ -e "/Users/$user/.aspera" ]; then
                      if [ "$(ls -A "/Users/$user/.aspera")" ] ; then
                          echo "Keeping /Users/$user/.aspera since it is not empty"
                      else
                          remove_directory "/Users/$user/.aspera"
                      fi
                  fi
                  remove_directory "/Users/$user/Library/Application Support/Aspera/Aspera Connect"
              fi
          fi
